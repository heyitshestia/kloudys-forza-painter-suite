...
(async()=>{
  ...
  try {
    ...
    await page.waitForFunction(()=>document.getElementById('private-log-state').textContent.includes('1 retained'));
    ...
    check((await page.locator('#private-log-state').innerText()).includes('1 retained'),'full logs survive sign-in redirect');
    ...
    await page.reload();await page.waitForFunction(()=>document.getElementById('private-log-state').textContent.includes('1 retained'));
