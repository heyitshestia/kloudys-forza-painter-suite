  ...
  for (const [family, index, count, throttle, reference = false] of [
    ...
    const fixture = await page.evaluate(async ({family, index, count, reference}) => {
      const shapes = Array.from({length: count}, (_, i) => {
        ...
        return { type: editorCatalog.resourceToTypeCode(f, slot), type_word: editorCatalog.resourceToShapeWord(f, slot),
      ...
      if (reference) {
        ...
        await editorReference.loadOverlayImageFromUrl(source.toDataURL("image/png"), "24MP-test-reference.png");
