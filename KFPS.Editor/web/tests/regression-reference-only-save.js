async page => {
  ...
  await page.waitForFunction(()=>editorReference.image && documentDirty);
  ...
  const before=await page.evaluate(()=>({page:KfpsEditorDiagnostics.snapshot().page,history:currentHistoryState()===savedHistoryState,ref:editorReference.sourceOverlayProjectState()}));
  ...
  await page.reload();
  await page.waitForFunction(()=>window.KfpsDesktop?.ready && editorReference.image);
  ...
  const after=await page.evaluate(()=>({page:KfpsEditorDiagnostics.snapshot().page,layers:vinylObjects().length,ref:editorReference.sourceOverlayProjectState()}));
