async (page, options = {}) => {
  ...
  if (options.project) {
    ...
    await page.evaluate(() => { window.guideOriginalShapes = JSON.stringify(snapshotShapes()); });
  } else await page.evaluate(async options => {
    const count = Number(options.layers || 1);
    await loadPayload({ shapes: Array.from({length:count},(_,i)=>({type: i===count-1?1048706:1048677,
      color: [70+i%120, 140, 220, 255], data: i===count-1?[0,0,2,2,25,0,0]:[-390+i%60*13,260-Math.floor(i/60)*11,.12,.12,i%4*15,0,0]})) });
    const source = document.createElement("canvas"); source.width = options.layers ? 6000 : 2400; source.height = options.layers ? 4000 : 1600;
    const ctx = source.getContext("2d"); ctx.fillStyle = "#92a5b0"; ctx.fillRect(0, 0, source.width, source.height);
    ctx.fillStyle = "#e7f0f0"; ctx.fillRect(200, 200, 1800, 60);
    await editorReference.loadOverlayImageFromUrl(source.toDataURL(), "guide-reference.png");
  ...
  await page.evaluate(point=> {
    window.guideTouchEvidence={point,hit:document.elementFromPoint(point.x,point.y)?.outerHTML?.slice(0,300),events:[]};
    for(const name of ['touchstart','touchmove','touchend','pointerdown','pointerup','mousedown','mouseup'])document.addEventListener(name,event=> {
      if(guideTouchEvidence.events.length<60)guideTouchEvidence.events.push({type:event.type,target:event.target.tagName,id:event.target.id,tool:activeToolMode,draft:!!guideDraft,pointer:event.pointerType});
    },{capture:true});
  },a);
  await cdp.send("Emulation.setTouchEmulationEnabled", {enabled:true,maxTouchPoints:1});
  try {
    await cdp.send("Input.dispatchTouchEvent", {type:"touchStart",touchPoints:[{x:a.x,y:a.y}]});
    await page.waitForFunction(()=>!!guideDraft,{},{timeout:3000});
    for(let step=1;step<=8;step++) {
      await cdp.send("Input.dispatchTouchEvent", {type:"touchMove",touchPoints:[{x:a.x+step*10,y:a.y+step*10}]});
      await page.waitForTimeout(16);
    }
    await cdp.send("Input.dispatchTouchEvent", {type:"touchEnd",touchPoints:[]});
    await page.waitForFunction(()=>!guideDraft&&guideState.guides.length===1,{},{timeout:3000});
    const touch = await state();
    assert(!touch.draft && touch.guides.length===1 && Number.isFinite(touch.guides[0].x2), "Touch guide drawing regressed: "+JSON.stringify(touch));
    assert(await page.evaluate(()=>guideOriginalShapes===JSON.stringify(vinylObjects().map(o=>objectToShape(o)))), "Touch guide drawing moved an underlying shape");
  } finally {
    fs.writeFileSync(path.join(output,'guide-touch-evidence.json'),JSON.stringify(await page.evaluate(()=>({...guideTouchEvidence,draft:guideDraft,guides:cloneGuidesForSave(),tool:activeToolMode,pan:isPanning})),null,2));
  ...
  return { layers:await page.evaluate(()=>vinylObjects().length), geometryCases, guides: completed, reference: true, pointerNavigation: true, preciseGuidePicking: true, touchDrag:true };
}
