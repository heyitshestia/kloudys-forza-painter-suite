async page => {
  ...
  for (const [opacity, throttle] of [[0,1],[.6,1],[1,1],[.6,4]]) {
    const fixture = await page.evaluate(opacity => {
      editorReference.image.set({opacity});
      ...
      canvas.renderAll();
      return {layers:vinylObjects().length, reference:[editorReference.image.width,editorReference.image.height], opacity,
    ...
    const metrics = await page.evaluate(() => {
      ...
      return {...result,frameP95:frames[Math.floor(frames.length*.95)],frameMax:Math.max(...frames),
        angle:reportObject.angle,layers:vinylObjects().length,hybridDisabled:editorRenderer.disabledReason};
    });
  ...
  const recovery=await page.evaluate(async()=>{await flushPendingAutosave();return editorRecovery.status;});
  await cdp.detach();
