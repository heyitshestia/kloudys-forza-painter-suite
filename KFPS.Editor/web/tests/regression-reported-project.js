async page => {
  ...
  await page.evaluate(async content => {
    ...
    window.reportedReference = editorReference.sourceOverlayProjectState().data_url;
    editorReference.image.set({opacity:.6});
  },content);
  const results=[];
  ...
  const persistence=await page.evaluate(async ids=>{
    ...
    if(editorReference.sourceOverlayProjectState().data_url!==reportedReference)throw new Error("Reference bytes changed");
    await flushPendingAutosave();
    if(!editorRecovery.status.serverOk||!editorRecovery.status.browserOk)throw new Error("Recovery not acknowledged");
    return {layers:vinylObjects().length,referencePreserved:true,recovery:editorRecovery.status};
  },results.map(r=>r.id));
