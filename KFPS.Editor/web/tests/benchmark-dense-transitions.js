...
async page => {
  ...
  for(const count of counts){
    ...
    await page.waitForFunction(n=>vinylObjects().length===n&&editorReference.image&&!documentDirty,count);
    await page.locator('#fitView').click();
    const loaded=await page.evaluate(()=>({layers:vinylObjects().length,reference:[editorReference.image.width,editorReference.image.height],visible:vinylObjects().filter(o=>o.isOnScreen()).length}));
