async page => {
  ...
  await page.evaluate(async () => {
    await loadPayload({shapes:[{type:editorCatalog.resourceToTypeCode("Primitives",1),data:[0,0,2,2,0,0,0],color:[120,180,210,255]}]});
    fitDesignView();
  });
  ...
  const savedRequest=await page.evaluate(async()=>{
    currentProjectName="PRIVATE_DIAGNOSTIC_PROJECT";
    await saveProject();
    if(documentDirty||!editorProjects.association)throw Error("Diagnostic save failed");
    const before=KfpsEditorDiagnostics.snapshot().metrics.totalDraws;
    for(let i=0;i<20;i++)canvas.renderTop();
    if(KfpsEditorDiagnostics.snapshot().metrics.totalDraws!==before)throw Error("Selection-only redraw counted as full canvas render");
    return editorProjects.association.request_id;
  });
  await page.waitForTimeout(2500);
  const records=fs.readFileSync(path.join(output,"profile","performance.jsonl"),"utf8").trim().split(/\r?\n/).map(JSON.parse);
  const events=records.flatMap(record=>record.kind==="sample"?record.events:[record]);
  const correlated=events.filter(event=>event.requestId===savedRequest&&event.job==="saveProject");
  for(const state of ["pending","committed","finished"])if(!correlated.some(event=>event.state===state))throw Error(`Save correlation missing ${state}`);
  if(!records.some(record=>record.kind==="native-sample"&&Number.isFinite(record.uiLag)))throw Error("Native timing was not written to disk");
  if(/PRIVATE_/.test(JSON.stringify(records)))throw Error("Private project text leaked into diagnostics");
  ...
  return {capturedStall:true,errorsAndWarnings:true,redacted:true,recovery:true,identity:true,limits,
    saveStates:correlated.map(event=>event.state),nativeTiming:true,selectionRenderPairing:true,logging:stored.logging};
}
