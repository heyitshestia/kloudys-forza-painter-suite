async (page) => {
  ...
  return page.evaluate(async () => {
    ...
    const runHistoryCase = async (count, withEditorIds = false) => {
      ...
      editorRenderer.beginHybridRender("history threshold regression");
    ...
    const runRapidHistoryCase = async () => {
      ...
      const expectedUndoIndex = editorHistory.index - 5;
      const expectedUndoState = comparableState(editorHistory.entries[expectedUndoIndex]);
      await Promise.all([undo(), undo(), undo(), undo(), undo()]);
      const undoMatches = editorHistory.index === expectedUndoIndex
        && comparableState(snapshotEditorState()) === expectedUndoState;
      const expectedRedoIndex = editorHistory.index + 3;
      const expectedRedoState = comparableState(editorHistory.entries[expectedRedoIndex]);
      await Promise.all([redo(), redo(), redo()]);
      const redoMatches = editorHistory.index === expectedRedoIndex
        && comparableState(snapshotEditorState()) === expectedRedoState;
      editorRenderer.endHybridRenderNow();
      return { undoMatches, redoMatches, historyIndex: editorHistory.index, expectedRedoIndex, state: stateSummary() };
    };
    ...
    const sampleHybridOverlay = async () => {
      editorRenderer.endHybridRenderNow();
      await loadPayload({ shapes: makeShapes(editorRenderer.minimumLayers) });
      ...
      await editorReference.loadOverlayImageFromUrl(marker.toDataURL("image/png"), "hybrid-orientation.png");
      overlayLayerMode = "above";
      editorReference.image.set({ left: 0, top: 0, scaleX: 120, scaleY: 120, angle: 0, opacity: 1 });
      editorReference.image.setCoords();
      editorRenderer.beginHybridRender("overlay orientation regression");
      editorRenderer.hybridRenderNow();

      const gl = editorRenderer.renderer.gl;
      const readLocal = (x, y) => {
        const world = fabric.util.transformPoint(new fabric.Point(x, y), editorReference.image.calcTransformMatrix());
        const screen = fabric.util.transformPoint(world, canvas.viewportTransform);
        const pixel = new Uint8Array(4);
        gl.readPixels(
          Math.max(0, Math.min(editorRenderer.renderer.element.width - 1, Math.round(screen.x))),
          Math.max(0, Math.min(editorRenderer.renderer.element.height - 1, editorRenderer.renderer.element.height - 1 - Math.round(screen.y))),
          1,
          1,
          gl.RGBA,
          gl.UNSIGNED_BYTE,
