...
async (page) => {
  ...
  for (const kind of ['move', 'resize', 'mask', 'selection', 'floor', 'skew']) {
    const start = await page.evaluate(async kind => {
      ...
      return { x: r.left + p.x, y: r.top + p.y, matrices: vinylObjects().map(o => o.calcTransformMatrix()), index: editorHistory.index };
    }, kind);
    await page.mouse.move(start.x, start.y);
    ...
    await page.evaluate(start => {
      ...
      testAssert(editorHistory.index === start.index, 'cancelling a drag consumed committed history');
    ...
    interruptedDrags.push(await page.evaluate(start => {
      testAssert(editorHistory.index === start.index, 'mouse release committed cancelled drag');
  ...
  const lifecycle = await page.evaluate(async () => {
    ...
    const originalLoader = editorCatalog.loadResourceOutlinePathForResolved;
    let finishOutline;
    try {
      editorCatalog.loadResourceOutlinePathForResolved = () => new Promise(resolve => { finishOutline = resolve; });
      ...
      editorCatalog.loadResourceOutlinePathForResolved = originalLoader;
    }
