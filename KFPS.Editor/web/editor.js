...
const EDITOR_UPDATE_ACK_KEY = "kloudyFabricEditorUpdateAcknowledged";
const EDITOR_UPDATE_NOTICE_VERSION = "modernization-1";
let editorUpdateConfirmationPending = false;
let editorUpdates = null;
...
const editorPersistence = KfpsEditorPersistence.create({ headers: EDITOR_MUTATION_HEADERS, maxBytes: EDITOR_PROJECT_MAX_BYTES }, {
  ...
  cause: () => currentRecoveryCause(),
  event: fields => window.KfpsEditorDiagnostics?.record("job", fields),
});
const editorProjects = KfpsEditorProjects.create({ persistence: editorPersistence, generation: () => documentIdentity });
const AUTOSAVE_IDLE_MS = 500;
const AUTOSAVE_MAX_WAIT_MS = 2000;
const editorRecovery = KfpsEditorRecovery.create({
  transport: editorPersistence, eligible: () => !recoveryRestoreDepth,
  idleMs: AUTOSAVE_IDLE_MS, maxWaitMs: AUTOSAVE_MAX_WAIT_MS,
  onQueued: revision => window.KfpsEditorDiagnostics?.queued(revision),
  onResult: reportAutosaveResult,
  onError: error => console.warn(KfpsI18n.t("App-folder autosave skipped."), error),
  onClear: revision => {
    try {
      localStorage.removeItem(AUTOSAVE_KEY);
      localStorage.setItem(AUTOSAVE_CLEAR_KEY, String(revision));
    } catch (_error) { /* Independent app/browser checkpoints still carry the clear. */ }
  },
});
...
const editorCatalog = KfpsEditorCatalog.create({ VINYL_TYPE_BASES, VINYL_RESOURCE_BASES, format: fmt, KfpsI18n });
...
const GRADIENT_SHAPE_WORDS = new Set(
  Object.entries(GRADIENT_RESOURCE_SLOTS).flatMap(([family, slots]) => (
    slots.map((index) => editorCatalog.resourceToShapeWord(family, index))
  ))
);
...
// Attempts cancel staged work; only an accepted replacement changes the save identity.
let documentIdentity = 0;
let contentCommitId = 0;
let documentFirstCommitId = 0;
let durableCommitId = 0;
let acceptedContentCause = null;
let editorTransformInputs = null;
let editorShutdownPromise = null;
...
const editorReference = KfpsEditorReference.create({
  scene: { canvas: () => canvas, discard: object => discardFabricObject(object),
    releasePreview: () => editorRenderer.releaseHybridOverlay(), prewarm: () => editorRenderer.hybridRenderNow(), order: () => layerEditorHelpers() },
  session: { generation: () => documentIdentity, toolMode: () => activeToolMode,
    layerMode: () => overlayLayerMode, setLayerMode: (...args) => setOverlayLayerMode(...args) },
  view: { element: id => $(id), hidden: (...args) => setHidden(...args), text: (...args) => setText(...args),
    status: message => setStatus(message), hud: () => updateHud(), interactivity: () => updateSourceInteractivity(),
    scaleControls: value => syncOverlayScaleControls(value),
    failed: error => showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error) },
  persistence: { request: (...args) => editorPersistence.request(...args) },
  changed: reason => markOverlayChanged(reason), maxReferenceBytes: EDITOR_REFERENCE_MAX_BYTES, fabric, KfpsI18n, round,
});
const editorRenderer = KfpsEditorRenderer.create({
  scene: { canvas: () => canvas, objects: () => vinylObjects(), selected: () => selectedVinylObjects(),
    visible: object => objectEditorVisible(object), toolMode: () => activeToolMode, generation: () => documentGeneration,
    documentIdentity: () => documentIdentity },
  reference: { get image() { return editorReference.image; }, layerMode: () => overlayLayerMode },
  view: { element: id => $(id), text: (...args) => setText(...args), mode: count => currentHudMode(count),
    hud: () => updateHud(), cssColor: (...args) => cssColorVar(...args) },
  catalog: { payloads: editorCatalog.payloads, key: resolved => editorCatalog.resourceCacheKey(resolved),
    isGradient: object => isGradientObject(object), alphas: (...args) => decodeVertexAlphas(...args) },
  geometry: { vertex: pointFromPayloadVertex, indices: payloadTriangleIndices },
  colors: { normalize: normalizeColor, parse: hexToRgb }, schedule: { nextFrame: () => nextFrame() },
  diagnostics: { record: (kind, fields) => recordEditorDiagnostic(kind, { ...currentRecoveryCause(), ...fields }),
    awaitPaint: phase => awaitEditorPaint(phase) },
  maxLayers: MAX_VINYL_LAYERS, KfpsI18n,
});
const editorTheme = KfpsEditorThemes.create({
  scene: { canvas: () => canvas, styleControls: () => styleAllTransformControls(), grid: () => updateVisualGridLayer() },
  view: { element: id => $(id), status: message => setStatus(message), error: (...args) => showError(...args) },
  settings: editorSettings, endpoints: { preferences: EDITOR_PREFS_API, themes: EDITOR_THEMES_API },
  headers: EDITOR_MUTATION_HEADERS, cleanName: cleanProjectBaseName, escapeHtml, KfpsI18n,
});
const editorHistory = KfpsEditorHistory.create();
...
let shapeGridGeneration = 0;
let shapeGridFrame = 0;
...
const editorCommands = KfpsEditorCommands.create({
  generation: () => documentGeneration,
  overflow: () => setStatus(KfpsI18n.t("The editor is busy. Wait for the queued edits to finish before adding more.")),
  event: item => window.KfpsEditorDiagnostics?.record("command", { action:item.name, state:item.state, commandId:item.id, documentGeneration, documentId:documentIdentity }),
});
const editorLayerGroups = KfpsEditorLayerGroups.create({
  scene: {
  ...
  edits: { generation: () => documentGeneration, commit: reason => pushHistory(reason) },
  view: {
    render: () => canvas.requestRenderAll(), refresh: () => refreshLayers(), selection: () => updateSelectionPanel(),
    status: message => setStatus(message),
    failed: error => showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error),
  },
  tr: (...args) => KfpsI18n.t(...args), requestName: (...args) => requestTextInput(...args),
});
...
function isGradientResource(resource) {
  ...
  const shapeWord = Number.isFinite(explicitWord)
    ? (explicitWord & 0xffff)
    : editorCatalog.resourceToShapeWord(resource.family, resource.index);
  if (GRADIENT_SHAPE_WORDS.has(shapeWord)) return true;
  if (ALPHA_MESH_IMAGE_FAMILIES.has(String(resource.family))) return false;
  const name = editorCatalog.names?.families?.[resource.family]?.[String(resource.index)] || "";
...
function applyObjectColor(object, color, options = {}) {
  ...
  if (isGradientObject(object) && fabric?.Image?.filters?.BlendColor) {
    if (options.deferImageFilter) {
      ...
      if (editorRenderer.active && canvas) editorRenderer.requestHybridRender();
      return;
    }
  ...
  if (editorRenderer.active && canvas) {
    editorRenderer.requestHybridRender();
  }
}
...
function ensureSelectionOutlinePath(object) {
  ...
  const cacheKey = editorCatalog.resourceCacheKey(resolved);
  if (selectionOutlinePathPromises.has(cacheKey)) return;
  const promise = editorCatalog.loadResourceOutlinePathForResolved(resolved)
    .then((outlinePath) => {
...
function updateHud(pointer = null, options = {}) {
  ...
  if (!options.pointerOnly) {
    ...
    setHidden("emptyCanvasHint", stats.count > 0 || Boolean(editorReference.image));
  }
  if (pointer) {
...
function currentHudMode(selectedCount = 0) {
  ...
  if (activeToolMode === "source") return editorReference.image ? KfpsI18n.t("Move reference image") : KfpsI18n.t("Move Reference - no image");
...
async function startBlankCanvas() {
  ...
  return loadPayload({ shapes: [] }, {
    strict: true, newCanvas: true, name: "untitled", projectName: null,
  });
}
...
function setToolRailMode(mode, label = null) {
  ...
  if (activeToolMode === "guides") editorRenderer.endHybridRenderNow();
...
function setActiveTool(button) {
  ...
  if (mode === "source") setStatus(editorReference.image ? KfpsI18n.t("Move Reference mode. Drag only the reference image; vinyl layers and guides are ignored. Hold Control to snap it to the grid or guides.") : KfpsI18n.t("Move Reference needs an image first. Add one in Reference controls."));
}
...
async function makeFabricObject(shape, name = null) {
  ...
  const typeResolved = editorCatalog.typeCodeToResource(typeCode);
  ...
  const payload = resolved ? await editorCatalog.loadResourcePayloadForResolved(resolved) : null;
  const d = resolved ? await editorCatalog.loadResourcePathForResolved(resolved) : await editorCatalog.loadResourcePath(typeCode);
  const color = normalizeColor(shape.color);
  ...
  const pathCacheKey = resolved ? `mesh:${editorCatalog.resourceCacheKey(resolved)}` : `mesh:${typeCode}`;
  ...
  const object = alphaMeshResource
    ...
    : imageTintResource
      ? await loadFabricImage(await editorCatalog.resolveVinylResourceUrl(resolved.family, resolved.index, ".png"))
  ...
  object.kloudy = {
    editor_id: editorId,
    name: name || (typeof shape.shape_name === "string" && shape.shape_name) || (resolved ? editorCatalog.shapeDisplayName(resolved.family, resolved.index) : typeLabel(typeCode)),
    type: typeCode,
...
function typeLabel(typeCode) {
  const resolved = editorCatalog.typeCodeToResource(typeCode);
  if (!resolved) return `Unknown ${typeCode}`;
  return editorCatalog.shapeDisplayName(resolved.family, resolved.index);
}
...
function localizedShapeDisplayName(family, index) {
  ...
  let word;
  try { word = editorCatalog.resourceToShapeWord(family, index); }
  catch (_) { word = editorCatalog.words?.families?.[family]?.[String(index)]; }
...
function localizedTypeLabel(typeCode) {
  const resource = editorCatalog.typeCodeToResource(typeCode);
...
function shapeSearchText(family, index, typeCode) {
  return [
    ...
    typeCode,
    `#${index}`,
    editorCatalog.shapeDisplayName(family, index),
...
async function loadShapeNames() {
  try {
    await editorCatalog.loadMetadata();
    renderShapeGrid();
  } catch (err) {
...
function previewEditorColor(color) {
  ...
  if (editable.length) editorRenderer.beginHybridRender("color preview");
  editable.forEach((obj) => applyObjectColor(obj, normalized, { deferImageFilter: editorRenderer.active }));
  refreshColorUiFast(normalized);
  syncSelectedShapeOutlines(selected);
  if (canvas && editable.length) {
    if (editorRenderer.active) {
      editorRenderer.requestHybridRender();
      editorRenderer.settleHybridRender(500);
...
function pickShapeColorFromEvent(opt) {
  ...
  if (overlayLayerMode === "above") {
    const overlayColor = editorReference.overlayColorAtCanvasPoint(pointer.x, pointer.y);
    if (overlayColor) {
  ...
  const color = editorReference.overlayColorAtCanvasPoint(pointer.x, pointer.y);
  if (!color) {
...
function captureAssetSelection() {
  return orderedSelectedVinylObjects().map(object => {
    const shape = objectToShape(object, { includeEditorMeta: true });
    // Reusable copies must not be deleted by regenerating the original pixel art.
...
function persistCollapsedLayerState() {
  ...
  if (editorHistory.index >= 0 && editorHistory.entries[editorHistory.index]) {
    const state = editorHistory.entries[editorHistory.index];
    if (state && typeof state === "object") {
      const nextState = { ...state, editor_collapsed_groups: collapsed };
      const changed = !historyStatesEqual(state, nextState);
      editorHistory.replaceCurrent(
        savedHistoryState && historyStatesEqual(savedHistoryState, nextState)
          ? savedHistoryState
          : nextState
      );
      if (changed) acceptEditorContent("group");
    }
  }
  try {
...
function historyStatesEqual(left, right) {
  return KfpsEditorHistory.equal(left, right);
}

function historyStorageEstimate() {
  return editorHistory.estimate();
}
...
async function restoreEditorState(snapshot, targetIndex = null) {
  ...
  const previousGuides = savedGuideState();
  const previousCollapsed = [...collapsedLayerGroups];
  let buildError = null;
  let committed = false;
  try {
    ...
    await editorRenderer.prewarmHybridMeshesForObjects(created);
    flushPendingNudgeHistory();
    if (generation !== documentGeneration || historyState !== currentHistoryState()) return false;
    if (targetIndex !== null && editorHistory.entries[targetIndex] !== snapshot) return false;
    ...
    targetObjects.forEach((object, index) => {
      ...
      if (reused.has(object) && object.__kloudyHistoryShape !== shape) {
        changed.set(object, objectToShape(object, { includeEditorMeta: true }));
        applyHistoryShapeToObject(object, shape);
      }
    });
    currentObjects.forEach((object) => { if (!reused.has(object)) canvas.remove(object); });
    setVinylStackOrder(targetObjects, { refresh: false });
    applyCollapsedLayerGroups(state.editor_collapsed_groups || []);
    applySavedGuideState(state.editor_guides || null, { refresh: false });
    if (targetIndex !== null && !editorHistory.acceptRestore(targetIndex, snapshot)) throw new Error("Stale history installation");
    committed = true;
    acceptEditorContent("undo");
    ...
    if (!committed) {
      if (generation === documentGeneration && historyLocked) {
        changed.forEach((shape, object) => applyHistoryShapeToObject(object, shape));
        currentObjects.forEach(object => { if (object.canvas !== canvas) canvas.add(object); });
        created.forEach(object => canvas.remove(object));
        setVinylStackOrder(currentObjects, { refresh: false });
        applyCollapsedLayerGroups(previousCollapsed);
        applySavedGuideState(previousGuides);
        const selection = currentObjects.filter(object => selectedIds.has(String(object.kloudy?.editor_id || "")));
        if (selection.length === 1) canvas.setActiveObject(selection[0]);
        else if (selection.length > 1) canvas.setActiveObject(styledActiveSelection(selection));
        canvas.requestRenderAll();
      }
      throw error;
    }
    window.KfpsEditorDiagnostics?.record("js-error", { error: "Error", action: "undo" });
    try { showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error); } catch (_) {}
    return true;
  } finally {
...
function resetHistory({ preserveGeneration = false } = {}) {
  ...
  editorHistory.reset();
  if (!preserveGeneration) acceptEditorContent("new");
...
function autosavePayloadFromState(state) {
  ...
  const sourceOverlay = editorReference.sourceOverlayProjectState();
...
function reportAutosaveResult(operation, browserOk, serverOk, error, code = "") {
  const cause = operation.editor_recovery_cause;
  if (serverOk && cause?.documentId === documentIdentity) durableCommitId = Math.max(durableCommitId, cause.commitId || 0);
  try { window.KfpsEditorDiagnostics?.recovery(operation.recovery_revision, browserOk, serverOk, cause, code); } catch (_) {}
  if (operation.recovery_revision !== editorRecovery.revision) return;
  const ok = browserOk || serverOk;
  if (operation.action === "clear" && ok) return;
  error = code === "recovery_too_large"
    ? KfpsI18n.t("Recovery exceeds the {0} MiB project limit. Use a smaller reference image.", EDITOR_PROJECT_MAX_BYTES / (1024 * 1024))
    : KfpsI18n.error(error);
...
function flushPendingAutosave() { return editorRecovery.flush(); }
...
function writeAutosavePayload(payload) {
  const queued = editorRecovery.write(payload);
  recordEditorDiagnostic("checkpoint", { ...payload?.editor_recovery_cause, revision: editorRecovery.revision,
    state: queued ? "pending" : "failed" });
  return queued;
}

function recordEditorDiagnostic(kind, fields) {
  try { window.KfpsEditorDiagnostics?.record(kind, fields); } catch (_) {}
}

function awaitEditorPaint(phase) {
  try { window.KfpsEditorDiagnostics?.awaitPaint(phase, currentRecoveryCause()); } catch (_) {}
}

function documentPhase(phase, started, generation, fields = {}) {
  recordEditorDiagnostic("phase", { documentId: documentIdentity, documentGeneration: generation,
    phase, state: "finished", duration: performance.now() - started, ...fields });
  return performance.now();
}

function currentRecoveryCause() {
  if (!acceptedContentCause || acceptedContentCause.documentId !== documentIdentity) return null;
  return { ...acceptedContentCause, firstCommitId: Math.min(acceptedContentCause.commitId,
    Math.max(documentFirstCommitId, durableCommitId + 1)) };
}

function acceptEditorContent(reason, action = "", inputId = 0) {
  const name = String(action || reason).toLowerCase();
  const mapped = [
    [/^(?:object edit|drag|move)/, "move"], [/^(?:field|numeric)/, "numeric"],
    [/rotate/, "rotate"], [/skew/, "skew"], [/scale|flip/, "scale"], [/duplicate/, "duplicate"],
    [/nudge/, "nudge"], [/undo|redo/, "undo"], [/reference/, "reference"], [/guide/, "guide"], [/^new$/, "new"],
    [/loaded|open project|recovered/, "load"], [/pixel/, "pixel"], [/text vinyl/, "text"],
    [/asset/, "asset"], [/group/, "group"], [/paste/, "paste"], [/delete/, "delete"],
    [/color|appearance|alpha/, "color"], [/mask/, "mask"], [/order/, "order"], [/align|distribute/, "layout"],
    [/add|insert|replace/, "add"],
  ].find(([pattern]) => pattern.test(name))?.[1] || "edit";
  if (acceptedContentCause?.documentId !== documentIdentity) { documentFirstCommitId = contentCommitId + 1; durableCommitId = 0; }
  let pageId;
  try { pageId = window.KfpsEditorDiagnostics?.pageId; } catch (_) {}
  acceptedContentCause = Object.freeze({ documentId: documentIdentity, commitId: ++contentCommitId,
    operationId: contentCommitId, commandId: editorCommands.committingId, historyId: editorHistory.sequence,
    pageId, inputId });
  recordEditorDiagnostic("commit", { ...acceptedContentCause, action: mapped, state: "committed" });
}
...
function clearAutosave() {
  ...
  return editorRecovery.clear();
}

function pushHistory(reason = "change", options = {}) {
  if (historyLocked) return;
  const previous = editorHistory.index >= 0 && typeof editorHistory.entries[editorHistory.index] === "object" ? editorHistory.entries[editorHistory.index] : null;
  ...
  if (!editorHistory.commit(snapshot, reason)) return;
  acceptEditorContent(reason, options.action, options.inputId);
  ...
  try {
    ...
    recordEditorDiagnostic("js-error", { error: "Error", action: "edit" });
    try { showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error); } catch (_) {}
  }
  return true;
}
...
function ensureHistoryBaseline() {
  if (historyLocked || editorHistory.index >= 0) return;
  const snapshot = captureSharedHistoryState();
  editorHistory.installBaseline(snapshot, "blank canvas", false);
  updateDocumentState();
  renderHistoryList();
}
...
function establishLoadedHistoryBoundary(reason = "loaded source", options = {}) {
  ...
  const snapshot = captureSharedHistoryState();
  pendingNudgeObjects.clear();
  editorHistory.installBaseline(snapshot, reason || "loaded source", true);
  savedHistoryState = null;
...
function currentHistoryState() {
  return editorHistory.index >= 0 ? editorHistory.entries[editorHistory.index] : null;
}

function hasEditableWorkspace() {
  return Boolean(canvas && (vinylObjects().length || editorReference.image || editorReference.unavailable
    || guideState.guides.length || currentProjectName));
}

function isDocumentDirty() {
  return hasEditableWorkspace() && (
    !currentProjectName
    || !savedHistoryState
    || currentHistoryState() !== savedHistoryState
...
function markOverlayChanged(reason = "reference image changed") {
  try { window.KfpsEditorDiagnostics?.pulse("reference"); } catch (_) {}
  overlayRevision += 1;
  acceptEditorContent("reference");
...
function renderHistoryList() {
  ...
  setText("historyPositionBadge", editorHistory.entries.length ? `${editorHistory.index + 1} / ${editorHistory.entries.length}` : "0 / 0");
  const floor = Math.max(0, editorHistory.floor);
  const canUndo = editorHistory.index > floor;
  const canRedo = editorHistory.index >= 0 && editorHistory.index < editorHistory.entries.length - 1;
  ...
  if (!editorHistory.entries.length) {
    const empty = document.createElement("p");
    empty.textContent = KfpsI18n.t("Changes will appear here after you import or add a shape.");
    list.appendChild(empty);
    return;
  }
  editorHistory.entries.forEach((state, index) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "historyEntry";
    button.classList.toggle("current", index === editorHistory.index);
    button.classList.toggle("protected", index === editorHistory.floor);
    button.disabled = index < floor;
    const reason = humanizeHistoryReason(
      state?.history_reason || (index === editorHistory.floor ? KfpsI18n.t("loaded source") : "change"),
    );
    button.innerHTML = KfpsI18n.t("\n      <span class=\"historyEntryIndex\">{0}</span>\n      <span class=\"historyEntryText\">\n        <b>{1}</b>\n        <small>{2} layers{3}</small>\n      </span>\n      <span class=\"historyEntryState\">{4}</span>\n    ", index + 1, escapeHtml(reason), state?.shapes?.length || 0, state?.history_at ? ` / ${escapeHtml(historyTimeLabel(state.history_at))}` : "", index === editorHistory.index ? KfpsI18n.t("Current") : (state === savedHistoryState ? KfpsI18n.t("Saved") : ""));
...
function jumpToHistory(index) {
  return queueEditorMutation(async () => {
    flushPendingNudgeHistory();
    const target = Math.max(Math.max(0, editorHistory.floor), Math.min(editorHistory.entries.length - 1, Number(index)));
    if (!Number.isInteger(target) || target === editorHistory.index) return;
    if (!await restoreEditorState(editorHistory.entries[target], target)) return;
    updateDocumentState();
    renderHistoryList();
    setStatus(KfpsI18n.t("Returned to {0}.", humanizeHistoryReason(editorHistory.entries[editorHistory.index]?.history_reason || "change")));
  });
}

function queueEditorMutation(operation, name = "edit") {
  return editorCommands.enqueue(operation, name);
}
...
async function undoNow() {
  ...
  const floor = Math.max(0, editorHistory.floor);
  if (editorHistory.index <= floor) {
    setStatus(editorHistory.floor >= 0 ? KfpsI18n.t("Undo stopped at loaded source.") : KfpsI18n.t("Nothing to undo."));
    return;
  }
  editorHistory.breakCoalescing();
  const target = editorHistory.index - 1;
  if (!await restoreEditorState(editorHistory.entries[target], target)) return;
  updateDocumentState();
  renderHistoryList();
...
function undo() {
  return queueEditorMutation(undoNow, "undo");
}
...
async function redoNow() {
  ...
  if (editorHistory.index >= editorHistory.entries.length - 1) return;
  editorHistory.breakCoalescing();
  const target = editorHistory.index + 1;
  if (!await restoreEditorState(editorHistory.entries[target], target)) return;
  updateDocumentState();
...
function redo() {
  return queueEditorMutation(redoNow, "redo");
}
...
function finishCanvasPan() {
  ...
  if (editorRenderer.active) {
    editorRenderer.requestHybridRender();
    editorRenderer.settleHybridRender();
  } else {
    canvas.requestRenderAll();
  }
}
...
function initCanvas() {
  ...
  KfpsFabricAdapter.installSceneRenderGate(canvas, () => editorRenderer.active
    && canvas.lowerCanvasEl.style.visibility === "hidden"
    && !editorRenderer.disabledReason);
  KfpsFabricAdapter.installCpuPixelPicking(canvas);
  KfpsFabricAdapter.installNearestControlPicking(canvas);
  KfpsFabricAdapter.installSelectionTranslationRestore(canvas);
  KfpsFabricAdapter.installInteractionOverlays(canvas, {
    active: () => editorRenderer.active && canvas.lowerCanvasEl.style.visibility === "hidden",
    helpers: () => editorGuideObjects(),
    onError: error => {
      editorRenderer.reset("Interaction overlay unavailable");
      recordEditorDiagnostic("preview-fallback", { code: 3 });
      recordEditorDiagnostic("js-error", { source: "editor-fabric-adapter.js", error: error?.name || "Error" });
      canvas.requestRenderAll();
    },
  });
  ...
  canvas.on("object:added", (event) => {
    ...
    if (event.target?.kloudy) {
      ...
      if (editorRenderer.active) editorRenderer.requestHybridRender();
    }
  });
  ...
  canvas.on("object:modified", (event) => {
    editorRenderer.settleHybridRender(80);
    ...
    pushHistory("object edit", { changedObjects: selectedVinylObjects(), validationScheduled: true,
      action: event.transform?.action, inputId: event.kfpsDiagnosticInputId });
    updateSelectionPanel();
    scheduleRefreshLayers({ geometryOnly: true });
  });
  canvas.on("object:moving", (event) => {
    if (event.target?.kloudyOverlay) {
      editorRenderer.beginHybridRender("source overlay move");
      constrainSourceOverlayTransform();
      snapSourceOverlayToGuides(event);
      if (editorRenderer.active) editorRenderer.requestHybridRender();
      return;
    }
    editorRenderer.beginHybridRender("move");
  ...
  ["object:scaling", "object:skewing"].forEach((eventName) => {
    canvas.on(eventName, (event) => {
      if (event.target?.kloudyOverlay) {
        editorRenderer.beginHybridRender("source overlay transform");
        constrainSourceOverlayTransform();
        snapSourceOverlayToGuides(event);
        if (editorRenderer.active) editorRenderer.requestHybridRender();
        return;
      }
      editorRenderer.beginHybridRender(eventName);
      leaveGuideModeForLayerEdit();
      const target = interactiveVinylTarget(event.target);
      if (target !== event.target) mirrorMaskProxyToOwner(event.target);
  ...
  canvas.on("object:rotating", (event) => {
    if (event.target?.kloudyOverlay) {
      editorRenderer.beginHybridRender("source overlay rotate");
      constrainSourceOverlayTransform();
      if (editorRenderer.active) editorRenderer.requestHybridRender();
      return;
    }
    editorRenderer.beginHybridRender("rotate");
    leaveGuideModeForLayerEdit();
    const target = interactiveVinylTarget(event.target);
    if (target !== event.target) mirrorMaskProxyToOwner(event.target);
  ...
  canvas.on("mouse:wheel", (opt) => {
    ...
    editorRenderer.beginHybridRender("zoom");
  ...
  canvas.on("mouse:down", (opt) => {
    ...
    if (activeToolMode === "guides") {
      ...
      if (opt.e.type?.startsWith("touch")) {
        // Fabric may already have armed a drag on the shape under the finger.
        // Guides own this gesture; disarming it does not commit a shape edit.
        KfpsFabricAdapter.cancelObjectTransform(canvas);
    ...
    if (activeToolMode === "source") {
      ...
      canvas.selection = false;
      transformAnchorSnapshot = null;
      if (opt.target !== editorReference.image) {
        canvas.setActiveObject(editorReference.image);
    ...
    if (opt.e.button === 1 || opt.e.button === 2) {
      ...
      canvas.skipTargetFind = true;
      transformAnchorSnapshot = null;
      editorRenderer.beginHybridRender("pan");
...
function resizeCanvas() {
  ...
  if (width !== lastCanvasSize.width || height !== lastCanvasSize.height) {
    ...
    editorRenderer.resizeHybridRenderer();
  }
  syncCanvasObjectCoords();
  updateVisualGridLayer();
  canvas.requestRenderAll();
...
function applySavedGuideState(saved = null, { refresh = true } = {}) {
...
function layerEditorHelpers() {
  if (editorReference.image && canvas.getObjects().includes(editorReference.image)) {
    if (overlayLayerMode === "above") KfpsFabricAdapter.bringObjectToFront(canvas, editorReference.image);
    else KfpsFabricAdapter.sendObjectToBack(canvas, editorReference.image);
  }
  syncMaskPreviewOutlines();
...
function setOverlayLayerMode(mode, options = {}) {
  const previous = overlayLayerMode;
  overlayLayerMode = normalizeOverlayLayerMode(mode);
  if (options.commit && editorReference.image && previous !== overlayLayerMode) markOverlayChanged("reference image adjusted");
  if (options.persist !== false) editorSettings.setItem(OVERLAY_LAYER_MODE_KEY, overlayLayerMode);
...
function configureOverlayForSourceMode(enabled) {
  if (!editorReference.image) return;
  editorReference.image.set({
...
function updateSourceInteractivity() {
  ...
  if (inSourceMode) {
    canvas.selection = false;
    canvas.skipTargetFind = false;
    if (editorReference.image) canvas.setActiveObject(editorReference.image);
    else canvas.discardActiveObject();
  }
  layerEditorHelpers();
  canvas.requestRenderAll();
...
function constrainSourceOverlayTransform() {
  if (!editorReference.image) return;
  const scale = Math.max(0.001, Math.max(Math.abs(Number(editorReference.image.scaleX) || 1), Math.abs(Number(editorReference.image.scaleY) || 1)));
  editorReference.image.set({
    scaleX: scale,
    scaleY: scale,
    skewX: 0,
...
function snapSourceOverlayToGuides(event = null) {
  if (!editorReference.image || activeToolMode !== "source") return false;
  constrainSourceOverlayTransform();
  ...
  const rect = editorReference.image.getBoundingRect(true, true);
  const xPoints = [
  ...
  editorReference.image.set({
    left: (editorReference.image.left || 0) + (bestX?.delta || 0),
    top: (editorReference.image.top || 0) + (bestY?.delta || 0),
  });
  editorReference.image.setCoords();
  clearSnapOverlay();
  const now = Date.now();
...
function finalizeAxisMoveSnap(target, bestX = null, bestY = null, contact = null) {
  if (!target || (!bestX && !bestY)) return null;
  target.setCoords();
  const rect = target.getBoundingRect(true, true);
  const activeContact = contact ? refreshedGuideContact(target, contact) : null;
  let deltaX = 0;
  let deltaY = 0;
  if (bestX?.line?.axis === "x") {
    const xPoints = axisSnapPointsForLine(rect, { source: "control", kind: bestX.point }, bestX.line).xPoints;
    const point = bestX.source === "guide" && activeContact?.point
      ? { value: activeContact.point.x }
      : xPoints.find((item) => item.kind === bestX.point) || xPoints[0];
  ...
  if (bestY?.line?.axis === "y") {
    ...
    const point = bestY.source === "guide" && activeContact?.point
      ? { value: activeContact.point.y }
      : yPoints.find((item) => item.kind === bestY.point) || yPoints[0];
...
function axisSnapPoints(rect, contact = null) {
  ...
  const xPoints = ["tl", "bl", "left"].includes(kind)
    ? [allX[0]]
    : (["tr", "br", "right"].includes(kind) ? [allX[2]] : [allX[1]]);
  const yPoints = ["tl", "tr", "top"].includes(kind)
    ? [allY[0]]
    : (["bl", "br", "bottom"].includes(kind) ? [allY[2]] : [allY[1]]);
...
function axisSnapPointsForLine(rect, contact, line) {
  ...
  // A rotated local edge and the world-aligned bounding box are different contacts.
  if (line?.source === "guide" && ["control", "preserved"].includes(contact?.source)
      && Number.isFinite(contact.point?.x) && Number.isFinite(contact.point?.y)) {
    const points = axisSnapPoints(rect, contact);
    return {
      xPoints: points.xPoints.map(point => ({ ...point, value: contact.point.x })),
      yPoints: points.yPoints.map(point => ({ ...point, value: contact.point.y })),
    };
  }
...
function applyAngledGuideSnap(target, contact, line, options = {}) {
  ...
  if (allowRotation && isSingleVinylShape && Number.isFinite(line.angle)) {
    const previousPoint = activeContact.point;
    target.set({ angle: guideAngleForContact(line, target, contact) });
    target.setCoords();
    activeContact = refreshedGuideContact(target, contact);
    KfpsFabricAdapter.rebaseDragContact(canvas, options.event?.target || target, previousPoint, activeContact.point);
  }
...
function snapTargetToGuides(target, event = null) {
  ...
  if (shouldUseLine) {
    overlayResult = applyAngledGuideSnap(target, contact, bestLine.line, { allowRotation: allowAngledRotation, event });
    ...
    if (transformAction === "move" && activeEdgeSnap && isSingleVinylShape) {
      ...
      if (rotationLine && rotationLine.source === "guide" && cursorNearAxisGuide && Number.isFinite(rotationLine.angle)) {
        const previousPoint = refreshedGuideContact(target, contact).point;
        ...
        KfpsFabricAdapter.rebaseDragContact(canvas, event?.target || target, previousPoint, rotatedContact.point);
        target.set({
    ...
    target.setCoords();
    finalizeAxisMoveSnap(target, bestX, bestY, contact);
    overlayContact = refreshedGuideContact(target,
      bestX?.source === "guide" || bestY?.source === "guide" ? contact : { kind: axisSnapContactKind(bestX, bestY) });
  }
...
async function loadSelectedProject() {
  ...
  try {
    ...
    if (!await loadProjectPayload(data.payload, entry.title || entry.name, { generation, receipt: data.receipt })) return;
...
async function loadStartupProjectFromQuery() {
  ...
  try {
    ...
    if (!await loadProjectPayload(data.payload, data.name || "project", { generation, receipt: data.receipt })) return true;
...
function presentAcceptedDocument(callback) {
  try { callback(); }
  catch (error) {
    recordEditorDiagnostic("js-error", { error: "Error", action: "load" });
    try { showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error); } catch (_) {}
  }
}
...
async function loadPayload(payload, options = {}) {
  ...
  phaseStarted = documentPhase("document-build", phaseStarted, generation, { layers: builtObjects.length });
  try {
    await editorRenderer.prewarmHybridMeshesForObjects(builtObjects);
    if (generation !== documentGeneration) {
      builtObjects.forEach(discardFabricObject);
      return false;
    }
  ...
  phaseStarted = documentPhase("document-prewarm", phaseStarted, generation);
  // Keep old backing stores until membership, coordinates and history can be accepted.
  const previousObjects = vinylObjects().slice();
  const previousOrder = canvas.getObjects().slice();
  const previousGuides = guideState;
  const previousCollapsed = collapsedLayerGroups;
  ...
  try {
    cancelEditorTransform();
    canvas.discardActiveObject();
    previousObjects.forEach(object => canvas.remove(object));
    builtObjects.forEach(object => canvas.add(object));
    applySavedGuideState(payload.editor_guides || null, { refresh: false });
    applyCollapsedLayerGroups(payload.editor_collapsed_groups || []);
    syncCanvasObjectCoords();
    baseline = captureSharedHistoryState();
    editorHistory.installBaseline(baseline, options.newCanvas ? "blank canvas" : options.historyReason || "loaded source", !options.newCanvas);
  } catch (error) {
    guideState = previousGuides;
    collapsedLayerGroups = previousCollapsed;
    builtObjects.forEach(object => { if (object.canvas === canvas) canvas.remove(object); });
    previousOrder.forEach((object, index) => {
      if (object.canvas !== canvas) canvas.add(object);
      canvas.moveTo(object, index);
    });
    if (previousSelection) canvas.setActiveObject(previousSelection);
    builtObjects.forEach(discardFabricObject);
    canvas.requestRenderAll();
    throw error;
  }
  ...
  editorProjects.associate(options.receipt, currentProjectName);
  editorReference.clearSourceOverlayState({ refresh: false });
  editorReference.preserveUnavailable(payload.editor_source_overlay || null);
  acceptEditorContent(options.newCanvas ? "new" : options.historyReason || "loaded source");
  documentDirty = isDocumentDirty();
  // New uses the same acceptance boundary, but retires recovery rather than saving a source.
  const recoveryClear = options.newCanvas ? clearAutosave() : null;
  if (!options.newCanvas) writeAutosavePayload(autosavePayloadFromState(baseline));
  phaseStarted = documentPhase("document-install", phaseStarted, generation);
  // Presentation and retirement cannot invalidate an accepted baseline or its receipt.
  previousObjects.forEach(object => presentAcceptedDocument(() => {
    removeVinylObjectHelpers(object); discardFabricObject(object);
  }));
  phaseStarted = documentPhase("document-retire", phaseStarted, generation, { objects: previousObjects.length });
  lastLayerListKey = null;
  dropperPreservedActiveObject = null;
  for (const refresh of [() => editorRenderer.endHybridRenderNow(), applyGuideStateToUi,
    renderGuideObjects, bringGuidesToBack, refreshLayers, options.newCanvas ? resetView : fitDesignView,
    updateSelectionPanel,
    () => editorRenderer.hybridRenderNow(), updateDocumentState, renderHistoryList, refreshExportValidation,
    () => { if (!editorRenderer.warmDocumentCaches()) awaitEditorPaint("document-paint"); }]) {
    presentAcceptedDocument(refresh);
  }
  presentAcceptedDocument(() => clearBusy(options.newCanvas
    ? KfpsI18n.t("Blank canvas ready. Open Shapes, Text, or Pixel to begin.")
    : KfpsI18n.t("Loaded {0}/{1} editable FH6 layer(s).{2}", builtObjects.length, normalized.length, failed ? KfpsI18n.t(" Failed: {0}.", failed) : "")));
  documentPhase("document-present", phaseStarted, generation);
  if (recoveryClear) await recoveryClear;
  return true;
}
...
function dropLayerBlockAtSlot(dragObjects, slot) {
  ...
  setVinylStackOrder(nextDisplay.slice().reverse(), { refresh: false });
...
function updateSelectionPanel() {
  const selected = selectedVinylObjects();
  const layerCount = vinylObjects().length;
  ...
  ["selectInverseLayers", "selectSameShape", "selectSameColor", "clearLayerSelection"].forEach((id) => {
    const el = $(id);
    if (el) el.disabled = id === "clearLayerSelection" ? selected.length < 1 : layerCount < 1;
  });
  if ($("selectAllLayers")) $("selectAllLayers").disabled = layerCount < 1;
  ...
  $("layerInfo").textContent = selected.length > 1
    ? KfpsI18n.t("{0} layer(s) selected. Drag the selection box to move them together. Color edits apply to unlocked selected layers.", selected.length)
    : KfpsI18n.t("{0} editable layer(s). Export writes bottom-to-top order.", layerCount);
  if (!enabled) {
...
function renderNumericEdit() {
  editorRenderer.beginHybridRender("numeric edit");
  requestCanvasRender();
  editorRenderer.settleHybridRender();
}
...
function applySelectionFields(options = {}) {
  ...
  obj.setCoords();
  const sampledColors = sampleLiveOverlayColors(obj);
  if (!options.preview) {
    pushHistory("field edit", { changedObjects: [obj] });
    rememberFontShapeTransform(obj);
  }
  rememberColor(color);
  presentLiveOverlayColors(sampledColors);
  syncMaskPreviewOutlines();
  renderNumericEdit();
  updateSelectionPanel();
  return true;
...
async function saveEditorJsonToAppFolder(name, payload) {
  return editorProjects.export(name, payload);
}

async function saveProjectToAppFolder(name, payload, expected = null) {
  try {
    return await editorProjects.save(name, payload, expected);
  } catch (error) {
  ...
  try {
    ...
    if (!exportName) {
      const requestedName = await requestTextInput(
        KfpsI18n.t("Export Vinyl JSON"), KfpsI18n.t("JSON name"), defaultName,
    ...
    if (generation !== documentGeneration) return;
    flushPendingNudgeHistory();
    if (refreshExportValidation().errors.length) return;
    payload = { shapes: vinylObjects().map(object => objectToShape(object, { includeEditorMeta: false })) };
    if (!payload.shapes.length) return;
    result = await saveEditorJsonToAppFolder(exportName, payload);
    if (generation === documentGeneration) {
      if (!currentProjectName) loadedName = exportName;
      setJsonBrowserSource("editor");
      await refreshJsonBrowser();
      if (generation === documentGeneration) selectJsonBrowserEntryById(result.id);
      setStatus(KfpsI18n.t("Exported {0} layer(s) to imgs/editor/{1}.", payload.shapes.length, result.name || `${exportName}.fh6-import.json`));
    }
    ...
    if (result?.receipt) {
      console.warn("Export saved; presentation refresh failed.", err);
      setStatus(KfpsI18n.t("The JSON was exported, but the list could not refresh. Your exported file is intact."));
    } else if (payload && await requestConfirmation(
      KfpsI18n.t("Export could not be confirmed"),
      KfpsI18n.t("Check Editor exports first. A file may already have been saved. Download a separate copy of this export?"),
      KfpsI18n.t("Download Copy"),
    )) {
      downloadText(filenameWithSuffix(exportName, "fh6-import"), JSON.stringify(payload, null, 2));
      setStatus(KfpsI18n.t("Downloaded a separate copy containing {0} layers.", payload.shapes.length));
    } else {
      setStatus(KfpsI18n.error(err.message || String(err)));
    }
  } finally {
...
async function saveProject(options = {}) {
  ...
  const generation = documentIdentity;
  ...
  try {
    ...
    if (generation !== documentIdentity) return;
    flushPendingNudgeHistory();
    ensureHistoryBaseline();
    const payload = editableProjectPayload(projectName);
    const savedRevision = { generation, history: currentHistoryState(), overlay: overlayRevision };
    const expected = !options.saveAs && cleanProjectBaseName(currentProjectName) === projectName
      ? editorProjects.expected(projectName) : null;
    result = await saveProjectToAppFolder(projectName, payload, expected);
    if (documentIdentity === savedRevision.generation) {
      ...
      editorProjects.associate(result.receipt, currentProjectName);
      try { flushPendingNudgeHistory(); }
      finally {
        documentDirty = isDocumentDirty();
        writeAutosavePayload(autosavePayloadFromState(nudgeHistoryPending ? snapshotEditorState() : currentHistoryState() || snapshotEditorState()));
      }
      updateDocumentState();
      renderHistoryList();
    ...
    if (result?.receipt) {
      console.warn("Project saved; presentation refresh failed.", err);
      setStatus(KfpsI18n.t("The project was saved, but the display could not refresh. Your saved file is intact."));
      return;
    }
...
async function loadProjectPayload(payload, displayName = "project", options = {}) {
  ...
  try {
    if (!await loadPayload({
      ...
      editor_guides: payload.editor_guides || null,
      editor_source_overlay: payload.editor_source_overlay || null,
    }, { generation, strict: true, name: projectName, projectName, receipt: options.receipt,
      historyReason: "open project" })) return false;
    ...
    try {
      referenceRestored = await editorReference.restoreSourceOverlayFromProject(payload.editor_source_overlay || null);
    } catch (err) {
      if (identity !== documentIdentity) return false;
      referenceError = err;
    ...
    if (identity !== documentIdentity) return false;
    flushPendingNudgeHistory();
    if (currentHistoryState() !== loadedHistory || referenceRestored === false && !referenceError) {
      restored = true;
      return true;
    }
    savedHistoryState = currentHistoryState();
    savedOverlayRevision = referenceError ? overlayRevision - 1 : overlayRevision;
    documentDirty = isDocumentDirty();
    presentAcceptedDocument(updateDocumentState);
    presentAcceptedDocument(renderHistoryList);
    recoveryAutosavePayload = null;
    restored = true;
    if (referenceError) {
    ...
    if (restored && identity === documentIdentity) writeAutosavePayload(autosavePayloadFromState(currentHistoryState() || snapshotEditorState()));
  }
}
...
function resolvedResourceForObject(object) {
  ...
  if (Number.isFinite(typeCode)) return editorCatalog.typeCodeToResource(typeCode);
  return null;
}
...
function renderGlobalShapeReplacePanel() {
  ...
  [...used.values()]
    ...
    .forEach((entry) => {
      ...
      const thumb = entry.family !== "Unknown" && entry.index ? editorCatalog.vinylResourceUrl(entry.family, entry.index, ".png") : "";
...
function orderedSelectedVinylObjects() {
  const selected = selectedVinylObjects();
  return KfpsEditorCore.orderByStack(selected, canvas.getObjects());
}
...
function insertDuplicateVinylObjects(clones, originals, mode) {
  ...
  const orderedOriginals = KfpsEditorCore.orderByStack(originals.filter(obj => members.has(obj)), all);
...
function updateShapeResourceOnShape(shape, family, index) {
  const typeCode = editorCatalog.resourceToTypeCode(family, index);
  const shapeWord = editorCatalog.resourceToShapeWord(family, index);
  return {
    ...shape,
    type: typeCode,
    type_word: shapeWord,
    ...
    shape_name: editorCatalog.shapeDisplayName(family, index),
  };
}
...
async function generatePixelArtRectangles() {
  ...
  try {
    ...
    const typeCode = editorCatalog.resourceToTypeCode("Primitives", 1);
    const shapeWord = editorCatalog.resourceToShapeWord("Primitives", 1);
...
async function loadTextVinylResourceMesh(resource) {
  ...
  const payload = await editorCatalog.loadResourcePayloadForResolved({
    ...resource, typeCode: resource.typeCode || editorCatalog.resourceToTypeCode(resource.family, resource.index),
  });
...
function textVinylPrimitiveResource(index) {
  return {
    family: "Primitives",
    index,
    typeCode: editorCatalog.resourceToTypeCode("Primitives", index),
    shapeWord: editorCatalog.resourceToShapeWord("Primitives", index),
  };
}
...
function buildTextVinylForzaLetterShapes(text, options, color, groupId, groupName) {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    for (const char of line) {
      ...
      const typeCode = editorCatalog.resourceToTypeCode(resource.family, resource.index);
      const shapeWord = editorCatalog.resourceToShapeWord(resource.family, resource.index);
      shapes.push({
        type: typeCode,
        type_word: shapeWord,
        resource_family: resource.family,
        resource_index: resource.index,
        shape_name: editorCatalog.shapeDisplayName(resource.family, resource.index),
        data: [
...
function buildTextVinylCurveShapes(runs, layout, color, groupId, groupName) {
  const square = {
    index: 1,
    typeCode: editorCatalog.resourceToTypeCode("Primitives", 1),
    shapeWord: editorCatalog.resourceToShapeWord("Primitives", 1),
  };
  const circle = {
    index: 2,
    typeCode: editorCatalog.resourceToTypeCode("Primitives", 2),
    shapeWord: editorCatalog.resourceToShapeWord("Primitives", 2),
  };
  const shapes = [];
  runs.forEach((run) => {
...
function renderShapeGrid() {
  ...
  const focusKey = grid.contains(active) ? active.closest(".shapeTile")?.dataset.resourceKey : null;
  const focusFavorite = Boolean(focusKey && active.classList.contains("favButton"));
  const generation = ++shapeGridGeneration;
  if (shapeGridFrame) cancelAnimationFrame(shapeGridFrame);
  shapeGridFrame = 0;
  grid.innerHTML = "";
  grid.setAttribute("aria-busy", "true");
  const favoritesOnly = showFavoritesOnly;
  const families = (query || favoritesOnly) ? FAMILY_ORDER : [selectedFamily];
  let familyCursor = 0, index = 1;
  const renderChunk = () => {
    if (generation !== shapeGridGeneration) return;
    shapeGridFrame = 0;
    try {
      const deadline = performance.now() + 4;
      const fragment = document.createDocumentFragment();
      let scanned = 0, added = 0, restoreFocus = null;
      // Broad queries can match the entire catalog. Bound both scanning and DOM
      // creation, and let newer input cancel unfinished work before the next frame.
      while (familyCursor < families.length && scanned < 64 && added < 12
        && (!scanned || performance.now() < deadline)) {
        const family = families[familyCursor];
        if (index > editorCatalog.shapeCountForFamily(family)) { familyCursor++; index = 1; continue; }
        const resourceIndex = index++;
        scanned++;
        const isFavorite = favorites.has(`${family}:${resourceIndex}`);
        if (favoritesOnly && !isFavorite) continue;
        const typeCode = editorCatalog.resourceToTypeCode(family, resourceIndex);
        if (query && !shapeSearchText(family, resourceIndex, typeCode).includes(query)) continue;
        const tile = createShapeLibraryTile(family, resourceIndex, typeCode, isFavorite);
        fragment.appendChild(tile);
        added++;
        if (tile.dataset.resourceKey === focusKey) restoreFocus = focusFavorite ? tile.querySelector(".favButton") : tile;
      }
      if (added) grid.appendChild(fragment);
      if (restoreFocus && document.activeElement === document.body) restoreFocus.focus({ preventScroll: true });
      if (familyCursor < families.length) shapeGridFrame = requestAnimationFrame(renderChunk);
      else grid.setAttribute("aria-busy", "false");
    } catch (error) {
      grid.setAttribute("aria-busy", "false");
      showError(KfpsI18n.t("Shape library refresh failed"), error);
    }
  };
  renderChunk();
}

function createShapeLibraryTile(family, index, typeCode, isFavorite) {
  const shapeWord = editorCatalog.resourceToShapeWord(family, index);
  const name = localizedShapeDisplayName(family, index);
  const tile = document.createElement("div");
  tile.className = `shapeTile${isFavorite ? " favorite" : ""}`;
  tile.dataset.resourceKey = `${family}:${index}`;
  tile.tabIndex = 0;
  tile.title = KfpsI18n.t("{0}\n{1} #{2}\nType {3} / word {4}", name, KfpsI18n.familyLabel(family), index, typeCode, shapeWord);
  tile.innerHTML = KfpsI18n.t("\n      <button class=\"favButton\" type=\"button\" title=\"{0}\">{1}</button>\n      <img alt=\"\" src=\"{2}\">\n      <span class=\"shapeName\">{3}</span>\n      <span class=\"shapeMeta\">{4} #{5}</span>\n      <span class=\"shapeWord\">word {6}</span>\n    ", isFavorite ? KfpsI18n.t("Remove favorite") : KfpsI18n.t("Add favorite"), isFavorite ? "x" : "+", "", escapeHtml(name), KfpsI18n.familyLabel(family), index, shapeWord);
  ...
  img.loading = "lazy";
  img.decoding = "async";
  img.src = editorCatalog.vinylResourceUrl(family, index, ".png");
  img.addEventListener("error", async () => {
    const fallback = await editorCatalog.resolveVinylResourceUrl(family, index, ".png");
...
function toggleFavorite(family, index) {
  ...
  const tile = $("shapeGrid").querySelector(`[data-resource-key="${CSS.escape(key)}"]`);
  if (!tile) return;
  const isFavorite = favorites.has(key);
  if (showFavoritesOnly && !isFavorite) {
    const next = tile.nextElementSibling || tile.previousElementSibling;
    const hadFocus = tile.contains(document.activeElement);
    tile.remove();
    if (hadFocus) (next?.querySelector(".favButton") || $("showAllShapes")).focus({ preventScroll: true });
    return;
  }
  // A favorite changes this tile, not the catalog. Rebuilding broad results
  // needlessly decodes thumbnails again and disrupts the scroll/focus anchor.
  tile.classList.toggle("favorite", isFavorite);
  const button = tile.querySelector(".favButton");
  button.title = KfpsI18n.t(isFavorite ? "Remove favorite" : "Add favorite");
  button.textContent = isFavorite ? "x" : "+";
}

async function duplicateSelectedNow(context) {
  if (historyLocked || recoveryRestoreDepth > 0) return false;
  flushPendingNudgeHistory();
  const generation = documentGeneration;
  const selected = selectedVinylObjects();
  ...
  try {
    ...
    inserted = true;
    context.commit(() => pushHistory(placement === "top" ? "duplicate" : `duplicate ${placement}`));
    committed = true;
    if (clones.length === 1) {
    ...
    if (generation !== documentGeneration) return;
    if (committed) {
      showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), err);
      return;
    }
...
function duplicateSelected() {
  return queueEditorMutation(duplicateSelectedNow, "duplicate");
}
...
function moveSelectedToEdge(front) {
  ...
  setVinylStackOrder(front ? remaining.concat(moving) : moving.concat(remaining), { refresh: false });
...
function setVinylStackOrder(order, { refresh = true } = {}) {
  if (!canvas) return;
...
function moveLayerBlock(objects, direction) {
  ...
  if (moved) setVinylStackOrder(order, { refresh: false });
  return moved;
}
...
function selectInverseLayers() {
  ...
  const inverse = vinylObjects().filter((object) => object.visible !== false && !selected.has(object));
  if (!inverse.length) clearLayerSelection();
  else selectObjects(inverse, KfpsI18n.t("inverse selection"));
}
...
function pasteCopiedLayers() {
  return queueEditorMutation(pasteCopiedLayersNow, "paste");
}

function nextLayerGroupName() { return editorLayerGroups.nextName(); }

function groupSelectedLayers() { return editorLayerGroups.group(); }

function renameSelectedLayer() { return editorLayerGroups.renameLayer(); }

function renameSelectedGroup() { return editorLayerGroups.renameGroup(); }

function ungroupSelectedLayers() { return editorLayerGroups.ungroup(); }

function toggleSelectedGroupVisibility() { return editorLayerGroups.toggleVisibility(); }

function toggleSelectedGroupLock() { return editorLayerGroups.toggleLock(); }
...
function nudgeSelected(dx, dy) {
  ...
  editorRenderer.beginHybridRender("nudge");
  const active = canvas.getActiveObject();
  ...
  const sampledColors = sampleLiveOverlayColors();
...
function applyOverlayColorToObject(obj, options = {}) {
  ...
  const color = editorReference.dominantOverlayColorForObject(obj);
  if (!color) {
  ...
  if (options.deferPresentation) return applied;
...
function sampleLiveOverlayColors(target = null) {
  if (!$("autoOverlayColor")?.checked || !editorReference.sampler) return [];
  const selected = selectedVinylObjects();
  const targets = unlockedObjects(selected.length ? selected : (target?.kloudy ? [target] : []));
  const colors = [];
  targets.forEach((obj) => {
    const color = applyOverlayColorToObject(obj, { deferPresentation: true, silent: true });
    if (color) colors.push(color);
  });
  return colors;
}

function presentLiveOverlayColors(colors) {
  if (colors.length) {
    colors.forEach(rememberColor);
    updateSelectionPanel();
    canvas.requestRenderAll();
  }
...
function applyLiveOverlayColor(target = null) {
  return presentLiveOverlayColors(sampleLiveOverlayColors(target));
}
...
function sampleOverlayColorForSelected() {
  ...
  const sampledColors = [];
  editable.forEach((obj) => {
    const sampled = applyOverlayColorToObject(obj, { deferPresentation: true, silent: true });
    if (sampled) { changed++; sampledColors.push(sampled); }
  });
  if (changed) pushHistory("reference color sample", { changedObjects: editable });
  sampledColors.forEach(rememberColor);
  canvas.requestRenderAll();
  updateSelectionPanel();
  if (changed) {
...
function bindTransformInputs() {
  editorTransformInputs?.dispose();
  editorTransformInputs = KfpsTransformInputs.create({
    scene: { selected: () => selectedVinylObjects()[0], contains: object => object.canvas === canvas,
      generation: () => documentGeneration, snapshot: object => objectToShape(object, { includeEditorMeta: true }),
      restore: applyHistoryShapeToObject },
    edits: { apply: applySelectionFields, flushNudge: flushPendingNudgeHistory,
      commit: object => {
        const committed = pushHistory("numeric drag", { changedObjects: [object] });
        rememberFontShapeTransform(object);
        return committed;
      } },
    view: { element: $, refresh: updateSelectionPanel, syncMasks: syncMaskPreviewOutlines,
      render: () => renderNumericEdit(), invalid: error => setStatus(KfpsI18n.error(error.message)),
      error: error => showError(KfpsI18n.t("The edit was kept, but the display could not refresh."), error) },
    parse: KfpsEditorCore.parseNumericExpression, round, document, window,
  });
}
...
async function continueEditorStartup() {
  ...
  if (maybeShowEditorUpdateNotice()) return;
...
function maybeShowEditorUpdateNotice() {
  if (editorSettings.getItem(EDITOR_UPDATE_ACK_KEY) === EDITOR_UPDATE_NOTICE_VERSION) return false;
  const dialog = $("editorUpdateDialog");
  if (!dialog) return false;
  if (dialog.open) return true;
  $("editorUpdateAcknowledge").checked = false;
  $("editorUpdateContinue").disabled = true;
  $("editorUpdateError").hidden = true;
  dialog.showModal();
  $("editorUpdateMessage").scrollTop = 0;
  return true;
}

async function confirmEditorUpdateNotice() {
  const checkbox = $("editorUpdateAcknowledge");
  if (!checkbox.checked || editorUpdateConfirmationPending) return;
  const button = $("editorUpdateContinue"), error = $("editorUpdateError");
  const previous = editorSettings.getItem(EDITOR_UPDATE_ACK_KEY);
  let saved = false;
  editorUpdateConfirmationPending = true;
  checkbox.disabled = button.disabled = true;
  error.hidden = true;
  try {
    editorSettings.setItem(EDITOR_UPDATE_ACK_KEY, EDITOR_UPDATE_NOTICE_VERSION);
    if (window.KfpsEditorPreferences && !await KfpsEditorPreferences.flush()) throw new Error("Preference write failed");
    if (location.protocol === "file:" && localStorage.getItem(EDITOR_UPDATE_ACK_KEY) !== EDITOR_UPDATE_NOTICE_VERSION) throw new Error("Preference write failed");
    saved = true;
  } catch (_) {
    if (previous === null) editorSettings.removeItem(EDITOR_UPDATE_ACK_KEY);
    else editorSettings.setItem(EDITOR_UPDATE_ACK_KEY, previous);
    error.textContent = KfpsI18n.t("Your acknowledgment could not be saved. Please try again.");
    error.hidden = false;
  } finally {
    editorUpdateConfirmationPending = false;
    checkbox.disabled = false;
    button.disabled = !checkbox.checked;
  }
  if (saved) {
    $("editorUpdateDialog").close();
...
async function readAutosavePayload() {
  ...
  try {
    ...
    editorRecovery.observe(data.highestRevision || 0);
  ...
  editorRecovery.observe(Math.max(clearedRevision, ...candidates.map(recoveryRevision)));
  return candidates
...
async function recoverAutosavePayload(payload) {
  ...
  try {
    if (!await loadPayload({
      ...
      editor_guides: payload.editor_guides || null,
      editor_source_overlay: payload.editor_source_overlay || null,
    }, { generation, strict: true, historyReason: "recovered work", name: cleanProjectBaseName(payload.name, "autosave"),
      projectName: payload.editor_session?.project_name ? cleanProjectBaseName(payload.editor_session.project_name) : null })) return false;
    identity = documentIdentity;
    restored = true;
    const loadedHistory = currentHistoryState();
    let referenceError = null;
    ...
    if (identity !== documentIdentity) return false;
    if (referenceError) editorReference.preserveUnavailable(payload.editor_source_overlay || null);
    flushPendingNudgeHistory();
    const recoveredState = loadedHistory;
    const recoveredOverlay = overlayRevision;
    const receipt = payload.editor_session?.receipt;
    const verified = await editorProjects.verify(receipt);
    if (identity !== documentIdentity) return false;
    if (verified) editorProjects.associate(receipt, currentProjectName);
    if (verified && payload.editor_session?.saved === true && currentProjectName && !referenceError && referenceRestored !== false
      && recoveredState === currentHistoryState() && recoveredOverlay === overlayRevision && !nudgeHistoryPending) {
      savedHistoryState = recoveredState;
      savedOverlayRevision = recoveredOverlay;
    }
    documentDirty = isDocumentDirty();
    presentAcceptedDocument(updateDocumentState);
    presentAcceptedDocument(renderHistoryList);
    ...
    if (identity !== null && identity !== documentIdentity) return false;
    clearBusy();
    updateDocumentState();
...
function bindUi() {
  $("editorUpdateDialog").addEventListener("cancel", event => event.preventDefault());
  $("editorUpdateAcknowledge").addEventListener("change", () => {
    $("editorUpdateContinue").disabled = !$("editorUpdateAcknowledge").checked || editorUpdateConfirmationPending;
  });
  $("editorUpdateContinue").addEventListener("click", confirmEditorUpdateNotice);
  ...
  editorAssetLibrary = KfpsEditorAssets.install({
    ...
    selection: captureAssetSelection,
    insert: shapes => queueEditorMutation(() => insertCopiedShapesNow(shapes, { asset: true }), "asset"),
  });
  bindTransformInputs();
  ...
  editorTheme.initialize(initialTheme);
  $("editorThemeSelect")?.addEventListener("change", (event) => editorTheme.applyEditorTheme(event.target.value));
  $("adjustTheme")?.addEventListener("click", editorTheme.openThemeAdjustDialog);
  $("themeAdjustBase")?.addEventListener("change", editorTheme.previewThemeAdjustBase);
  $("resetThemeAdjust")?.addEventListener("click", editorTheme.previewThemeAdjustBase);
  ...
  $("closeThemeAdjust")?.addEventListener("click", () => editorTheme.closeThemeAdjustDialog());
  $("themeAdjustDialog")?.addEventListener("cancel", (event) => {
    event.preventDefault();
    editorTheme.closeThemeAdjustDialog();
  });
  $("saveThemeAdjust")?.addEventListener("click", editorTheme.saveAdjustedTheme);
  ...
  $("newCanvas")?.addEventListener("click", () => {
    startBlankCanvas().catch(error => showError(KfpsI18n.t("Editor operation failed"), error));
  });
  ...
  $("overlayInput").addEventListener("change", (event) => {
    const file = event.target.files?.[0];
    if (file) editorReference.addOverlayFile(file);
    event.target.value = "";
  });
  ...
  $("overlayScalePercent")?.addEventListener("change", editorReference.updateOverlay);
  ...
  if ($("overlayLayerMode")) {
    ...
    $("overlayLayerMode").addEventListener("change", (event) => {
      setOverlayLayerMode(event.target.value, { commit: true });
    });
  }
  $("overlaySvgViewMode")?.addEventListener("change", (event) => editorReference.setLayeredOverlayViewMode(event.target.value));
  $("overlaySvgLayerSelect")?.addEventListener("change", (event) => editorReference.setLayeredOverlayLayer(Number(event.target.value)));
  $("overlaySvgPrevLayer")?.addEventListener("click", () => editorReference.setLayeredOverlayLayer((editorReference.layered?.selectedIndex || 0) - 1));
  $("overlaySvgNextLayer")?.addEventListener("click", () => editorReference.setLayeredOverlayLayer((editorReference.layered?.selectedIndex || 0) + 1));
  ...
  $("toggleOverlay").addEventListener("click", editorReference.toggleOverlay);
  $("removeOverlay").addEventListener("click", editorReference.removeOverlay);
...
async function executeDesktopOperation(operation, payload = {}) {
  if (operation === "state") {
    flushPendingNudgeHistory();
    return { dirty: documentDirty, saving: editorCommands.busy || recoveryRestoreDepth > 0 || pixelArtGenerationRunning || textVinylGenerationRunning || projectSaveInProgress || exportSaveInProgress || Boolean(editorAssetLibrary?.busy) };
  }
  if (operation === "close") {
    if (editorCommands.busy) return { ok: false, error: KfpsI18n.t("Wait for the current editor operation to finish before closing.") };
    ...
    await window.KfpsEditorDiagnostics?.drain();
    ...
    if (hasEditableWorkspace() && editorRecovery.status.serverOk !== true) return { ok: false, error: KfpsI18n.t("The latest recovery has not reached the app folder. Save the project before closing.") };
    return { ok: true };
  }
  ...
  if (payload.project) {
    ...
    if (!await loadProjectPayload(data.payload, data.name || "project", { generation, receipt: data.receipt })) return { cancelled: true };
...
window.KfpsDesktop = KfpsDesktopProtocol.create({
  execute: executeDesktopOperation,
  bridge: () => window.KfpsDesktopBridge,
  userWaiting: () => Boolean(document.querySelector("dialog[open]")),
  onError: err => { if (!window.KfpsDesktopBridge) showError(KfpsI18n.t("Editor operation failed"), err); },
});
function shutdownEditor() {
  if (editorShutdownPromise) return editorShutdownPromise;
  const drain = editorRecovery.shutdown();
  for (const owner of [editorUpdates, editorTransformInputs, window.KfpsDesktop, editorCommands, editorReference, editorRenderer, editorCatalog, editorTheme]) {
    try { owner?.dispose(); }
    catch (_) { window.KfpsEditorDiagnostics?.record("js-error", { error: "Error", action: "idle" }); }
  }
  // Native close already awaits recovery. Browser teardown is only best effort;
  // do not abort an accepted final write while the page is still able to finish it.
  editorShutdownPromise = drain.then(async () => {
    editorPersistence.dispose();
    try { await window.KfpsEditorDiagnostics?.drain(); }
    finally { window.KfpsEditorDiagnostics?.dispose(); }
  });
  return editorShutdownPromise;
}
window.addEventListener("pagehide", event => { if (!event.persisted) void shutdownEditor(); });
...
async function startEditor() {
  ...
  initCanvas();
  // An empty restart needs an undo origin before the first interactive insertion.
  editorHistory.installBaseline(captureSharedHistoryState(), "blank canvas", false);
  // New and explicit-project startup also need the floor, without decoding an old reference.
  const recoveryHead = await editorPersistence.request("recoveryHead");
  editorRecovery.observe(recoveryHead.highestRevision || 0);
  try {
    const floor = Number(localStorage.getItem(AUTOSAVE_CLEAR_KEY));
    editorRecovery.observe(floor);
  }
  catch (_err) { /* The worker's independent stores still establish the floor. */ }
  buildShapeLibrary();
  bindUi();
  editorUpdates = KfpsEditorUpdates.create({
    button: $("editorUpdateIndicator"), preferences: editorSettings, i18n: KfpsI18n,
    headers: EDITOR_MUTATION_HEADERS,
  });
  editorUpdates.start();
...
function editorDiagnosticState() {
  const active = canvas?.getActiveObject();
  const element = editorReference.image?.getElement?.();
  let cachePixels = 0, cacheCount = 0;
  for (const object of vinylObjectRegistry.objects) {
    const cache = object._cacheCanvas;
    if (cache) { cacheCount++; cachePixels += cache.width * cache.height; }
  }
  return {
    ...currentRecoveryCause(), cachePixels, cacheCount, meshCount: editorRenderer.meshCount,
    ...
    width: canvas?.width || 0, height: canvas?.height || 0, history: editorHistory.entries.length,
    renderer: editorRenderer.disabledReason ? "fallback" : editorRenderer.active ? "gpu-preview" : canvas ? "fabric" : "starting",
    referenceWidth: element?.naturalWidth || element?.width || 0,
    referenceHeight: element?.naturalHeight || element?.height || 0,
    referenceChars: (editorReference.source?.dataUrl?.length || 0) + (editorReference.source?.svgText?.length || 0),
    referenceAbove: overlayLayerMode === "above", referenceVisible: Boolean(editorReference.image && editorReference.image.visible !== false),
    referenceOpacity: editorReference.image?.opacity || 0,
  };
}
