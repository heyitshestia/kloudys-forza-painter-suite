(function installKfpsFabricAdapter(global) {
  ...
  function rebaseDragContact(canvas, target, before, after) {
    const transform = canvas?._currentTransform;
    if (!transform || !target || transform.target !== target || transform.action !== "drag") return false;
    const values = [before?.x, before?.y, after?.x, after?.y, transform.offsetX, transform.offsetY];
    if (!values.every(Number.isFinite)) return false;
    // Fabric places a drag at pointer - offset. A guide-induced rotation must
    // preserve the grabbed contact on the next pointer event, not the old center.
    transform.offsetX += after.x - before.x;
    transform.offsetY += after.y - before.y;
    return true;
  }
  ...
  const translationRestoreCanvases = new WeakSet();
  let translationRestoreInstalled = false;

  function installSelectionTranslationRestore(canvas) {
    const prototype = runtime.ActiveSelection?.prototype;
    if (typeof prototype?._restoreObjectsState !== "function") return false;
    translationRestoreCanvases.add(canvas);
    if (translationRestoreInstalled) return true;
    const original = prototype._restoreObjectsState;
    prototype._restoreObjectsState = function () {
      const matrix = this.calcOwnMatrix();
      if (!translationRestoreCanvases.has(this.canvas) || matrix[0] !== 1 || matrix[1] !== 0
        || matrix[2] !== 0 || matrix[3] !== 1 || !matrix.every(Number.isFinite)) {
        return original.call(this);
      }
      // An untouched or merely moved selection needs only translation. Fabric's
      // general decomposition otherwise rewrites mirrored scales and rotation
      // during deselection, creating an unrecorded edit to saved shape values.
      this._objects.forEach(object => {
        object.set({ left: object.left + matrix[4], top: object.top + matrix[5] });
        delete object.group;
        object.setCoords();
      });
      return this;
    };
    translationRestoreInstalled = true;
    return true;
  }

  const interactionOverlayCanvases = new WeakMap();

  function installInteractionOverlays(canvas, options) {
    if (!canvas?.on || !canvas.contextTop || typeof canvas.drawControls !== "function") return false;
    if (interactionOverlayCanvases.has(canvas)) {
      interactionOverlayCanvases.set(canvas, options);
      return true;
    }
    interactionOverlayCanvases.set(canvas, options);
    canvas.on("after:render", event => {
      const owner = interactionOverlayCanvases.get(canvas);
      if (!owner.active() || (event?.ctx && event.ctx !== canvas.contextContainer)) return;
      const context = canvas.contextTop;
      try {
        // Full Fabric frames clear the top surface; renderTop already prepared it.
        // Only guides and controls belong here, never the hidden vinyl scene.
        if (event?.ctx === canvas.contextContainer) {
          canvas.clearContext(context);
          canvas.renderTopLayer(context);
        }
        context.save();
        try {
          context.transform(...canvas.viewportTransform);
          for (const helper of owner.helpers()) {
            if (helper.canvas === canvas && helper.visible !== false) helper.render(context);
          }
  ...
  function withUncachedHitRender(object, render) {
    const caching = object.objectCaching, ownCaching = object.ownCaching, dirty = object.dirty;
    // Fabric's uncached render drops its normal drawing surface and clears dirty.
    // Picking is a read: retain that surface and restore invalidation afterwards.
    // Required clipping caches still follow Fabric's own lifecycle.
    const cache = !object.needsItsOwnCache?.() ? {
      _cacheCanvas: object._cacheCanvas, _cacheContext: object._cacheContext,
      cacheWidth: object.cacheWidth, cacheHeight: object.cacheHeight,
    } : null;
    object.objectCaching = false;
    try { return render(); }
    finally {
      if (cache) Object.assign(object, cache);
      object.objectCaching = caching;
      object.ownCaching = ownCaching;
      object.dirty = dirty;
    }
  }
  ...
  function installCpuPixelPicking(canvas) {
    ...
    canvas.isTargetTransparent = function (object, x, y) {
      ...
      try {
        ...
        return withUncachedHitRender(object, () => original.call(this, object, x, y));
      } catch (error) {
  ...
  function visiblePixelAt(canvas, object, point) {
    const bounds = sceneBounds(object);
    ...
    try {
      ...
      withUncachedHitRender(object, () => object.render(context));
      return context.getImageData(1, 1, 1, 1).data[3] > 0;
    } catch (error) {
      // A throwing Fabric render leaves an unmatched context save/transform.
      // Reset the private picking surface so the next hit starts in clean space.
      hitSurface.width = 3;
      throw error;
    } finally {
  ...
  global.KfpsFabricAdapter = Object.freeze({
    installInteractionOverlays,
    installSelectionTranslationRestore,
