...
if str(ROOT / "KFPS.Editor/src") not in sys.path:
...
from kfps_editor.recovery import RecoveryStore
from kfps_editor.projects import ProjectStore, ProjectSaveError, read_bounded_document
from kfps_editor.diagnostics import DiagnosticBusy, EditorDiagnostics, MAX_REQUEST as DIAGNOSTIC_MAX_REQUEST
from kfps_editor.manifest import editor_web_root, load_manifest

WEB_ROOT = editor_web_root(ROOT)
EDITOR = WEB_ROOT / "index.html"
SOURCE_MANIFEST = load_manifest(ROOT, verify_files=False)
STATIC_ASSETS = frozenset(item["path"] for item in SOURCE_MANIFEST["web"])
...
PROJECT_RECEIPT_API = "/api/fabric-editor/project-receipt"
...
VINYL_RESOURCE_ROOT = WEB_ROOT / "Resources" / "Vinyls"
SHAPE_WORDS_PATH = WEB_ROOT / "shape-words.json"
PREVIEW_MAX = 420
...
class ServerPaths(NamedTuple):
    """Immutable writable locations; application resources stay installation-owned."""
    startup_help: Path
    preferences: Path
    autosave: Path
    server_marker: Path
    ...
    @classmethod
    def for_runtime(cls, app_root: Path, runtime: Path, *, exports: Path | None = None):
        runtime = Path(runtime).resolve()
        return cls(*(runtime / name for name in (
            "startup-help-confirmed.json", "preferences.json", "autosave.json", "server.json",
            "editor-output-change.json", "project-change.json", "themes", "projects", "assets")),
            Path(exports).resolve() if exports is not None else Path(app_root).resolve() / "imgs/editor")

    @classmethod
    def legacy(cls):
        # Compatibility callers may customize module constants before construction.
        # Running servers capture these once; later rebinding cannot redirect them.
        return cls(STARTUP_HELP_MARKER, EDITOR_PREFS_MARKER, EDITOR_AUTOSAVE_MARKER,
            EDITOR_SERVER_MARKER, EDITOR_OUTPUT_CHANGE_MARKER, EDITOR_PROJECT_CHANGE_MARKER,
            EDITOR_THEME_ROOT, EDITOR_PROJECT_ROOT, EDITOR_ASSET_ROOT, EDITOR_JSON_ROOT)


def _read_preferences(*, paths: ServerPaths | None = None) -> dict:
    paths = paths if paths is not None else ServerPaths.legacy()
    try:
        payload = json.loads(paths.preferences.read_text(encoding="utf-8"))
...
def _asset_path(asset_id: str, *, paths: ServerPaths | None = None) -> Path:
    paths = paths if paths is not None else ServerPaths.legacy()
    if not isinstance(asset_id, str) or not re.fullmatch(r"[a-f0-9]{32}", asset_id):
        raise ValueError("Invalid asset ID.")
    path = (paths.assets / f"{asset_id}.asset.json").resolve()
    if path.parent != paths.assets.resolve():
...
def _read_asset(asset_id: str, *, paths: ServerPaths | None = None) -> dict:
    paths = paths if paths is not None else ServerPaths.legacy()
    path = _asset_path(asset_id, paths=paths)
...
def _asset_entries(*, paths: ServerPaths | None = None) -> dict:
    paths = paths if paths is not None else ServerPaths.legacy()
    entries, unavailable = [], []
    for path in paths.assets.glob("*.asset.json"):
        asset_id = path.name.removesuffix(".asset.json")
        try:
            entries.append(_asset_entry(_read_asset(asset_id, paths=paths)))
...
def _static_file(raw_path: str) -> Path | None:
    decoded = unquote(str(raw_path or ""))
    if any(value in decoded for value in ("\\", "\x00", ":")) or ".." in decoded.split("/"):
        return None
    if decoded == "/assets/kfps-logo.ico":
        return ROOT / "assets" / "kfps-logo.ico"
    mount = SOURCE_MANIFEST["legacy_web_mount"]
    if not decoded.startswith(mount):
        return None
    relative = decoded[len(mount):]
    if relative not in STATIC_ASSETS and not relative.startswith("Resources/Vinyls/"):
        return None
...
def _is_allowed_static_path(raw_path: str) -> bool:
    return _static_file(raw_path) is not None
...
def _resolve_browser_id(path_id: str, *, paths: ServerPaths | None = None) -> Path:
    paths = paths if paths is not None else ServerPaths.legacy()
    if not path_id or "\x00" in path_id:
        raise ValueError("missing JSON id")
    candidate = (ROOT / path_id).resolve()
    allowed_roots = [GENERATED_ROOT.resolve(), paths.exports.resolve(), EXPORTED_JSON_ROOT.resolve()]
...
def _resolve_project_id(path_id: str, *, paths: ServerPaths | None = None) -> Path:
    paths = paths if paths is not None else ServerPaths.legacy()
    if not path_id or "\x00" in path_id:
        raise ValueError("missing project id")
    candidate = (paths.projects / path_id).resolve()
    if not candidate.is_relative_to(paths.projects.resolve()):
...
def _theme_entries(*, paths: ServerPaths | None = None) -> list[dict]:
    paths = paths if paths is not None else ServerPaths.legacy()
    entries = [
    ...
    if not paths.themes.exists():
        return entries
    for path in sorted(paths.themes.glob("*.json"), key=lambda item: item.stat().st_mtime, reverse=True):
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
...
def _theme_exists(theme_id: str, *, paths: ServerPaths | None = None) -> bool:
    paths = paths if paths is not None else ServerPaths.legacy()
    if theme_id in {"pastel", "dark", "blackout", "whiteout"}:
        return True
    if not theme_id or "\x00" in theme_id:
        return False
    candidate = (paths.themes / f"{theme_id}.json").resolve()
    return candidate.is_relative_to(paths.themes.resolve()) and candidate.is_file()


def _unique_theme_id(base_id: str, *, paths: ServerPaths | None = None) -> str:
    paths = paths if paths is not None else ServerPaths.legacy()
    theme_id = base_id
    for index in range(2, 10000):
        target = (paths.themes / f"{theme_id}.json").resolve()
        if target.is_relative_to(paths.themes.resolve()) and not target.exists():
            return theme_id
...
def _unique_editor_export_path(base_name: str, *, paths: ServerPaths | None = None) -> Path:
    paths = paths if paths is not None else ServerPaths.legacy()
    clean = _clean_filename_base(base_name)
    return _unique_json_path(paths.exports / clean, clean)


def _project_path(base_name: str, *, paths: ServerPaths | None = None) -> Path:
    paths = paths if paths is not None else ServerPaths.legacy()
    clean = _clean_filename_base(base_name, "project")
    paths.projects.mkdir(parents=True, exist_ok=True)
    return paths.projects / f"{clean}.fabric-project.json"
...
def _editor_groups(*, paths: ServerPaths | None = None) -> list[dict]:
    paths = paths if paths is not None else ServerPaths.legacy()
    groups: dict[str, dict] = {}
    if not paths.exports.exists():
        return []
    for path in paths.exports.rglob("*.json"):
        if _is_internal_json(path):
            continue
        try:
            rel = path.relative_to(paths.exports)
        except ValueError:
            continue
...
def _project_entry(path: Path, *, paths: ServerPaths | None = None) -> dict:
    paths = paths if paths is not None else ServerPaths.legacy()
    stat = path.stat()
    name = path.name
    ...
    try:
        ...
        layers = 0
    return {
        "id": path.relative_to(paths.projects).as_posix(),
        "name": name,
        "title": title,
...
def _project_entries(*, paths: ServerPaths | None = None) -> list[dict]:
    paths = paths if paths is not None else ServerPaths.legacy()
    if not paths.projects.exists():
        return []
    entries = []
    for path in paths.projects.rglob("*.fabric-project.json"):
        try:
            entries.append(_project_entry(path, paths=paths))
        except Exception:
            continue
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        candidate = _static_file(urlparse(path).path)
        if candidate is None:
            raise ValueError("Static editor path is not permitted")
        return str(candidate)

    def do_HEAD(self):
        if not _is_allowed_static_path(urlparse(self.path).path):
            self.send_error(404)
            return
        super().do_HEAD()
    ...
    def _send_json(self, payload: dict, status: int = 200) -> bool:
        ...
        try:
            ...
            # A closed page cannot acknowledge a completed write. Its existing
            # receipt remains authoritative; never attempt a second response.
            return False
        return True

    def _record_save_commit(self, job: str, result: dict, started: float) -> None:
        diagnostics = getattr(self.server, "_diagnostics", None)
        if diagnostics is not None:
            try:
                diagnostics.record("job", job=job, state="committed", source="native",
                                   requestId=result["receipt"]["request_id"],
                                   duration=(time.perf_counter() - started) * 1000)
            except Exception:
                # Observability must not turn a successful save into a failure.
                pass
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == EDITOR_DIAGNOSTICS_API:
            ...
            return
        if parsed.path == "/api/fabric-editor/update-status":
            if not self._mutation_authorized():
                self._send_json({"error": "editor session authorization failed"}, status=403)
                return
            self._send_json(self.server.update_status)
            return
        ...
        if parsed.path == STARTUP_HELP_API:
            self._send_json({
                "confirmed": paths.startup_help.exists(),
                "marker": str(paths.startup_help),
            })
            return
        if parsed.path == EDITOR_PREFS_API:
            try:
                payload = _read_preferences(paths=paths)
            ...
            self._send_json({
                "theme": theme if _theme_exists(theme, paths=paths) else None,
                "settings": settings,
                "marker": str(paths.preferences),
            })
            return
        if parsed.path == EDITOR_THEMES_API:
            self._send_json({
                "themes": _theme_entries(paths=paths),
                "folder": str(paths.themes),
            })
            return
        if parsed.path == EDITOR_AUTOSAVE_API:
            query = parse_qs(parsed.query)
            compact = (query.get("compact") or [""])[0] == "1"
            with self.server.autosave_lock:
                store = self.server.recovery_store()
                head = store.head()
        ...
        if parsed.path == JSON_BROWSER_API:
            ...
            if source == "editor":
                groups = _editor_groups(paths=paths)
        ...
        if parsed.path == JSON_FILE_API:
            query = parse_qs(parsed.query)
            try:
                path = _resolve_browser_id((query.get("id") or [""])[0], paths=paths)
                payload = json.loads(read_bounded_document(path, EDITOR_PROJECT_MAX_BYTES))
        ...
        if parsed.path == PROJECT_BROWSER_API:
            entries = _project_entries(paths=paths)
            self._send_json({
                "entries": entries,
                "total_entries": len(entries),
                "folder": str(paths.projects),
            })
            return
        if parsed.path == PROJECT_FILE_API:
            query = parse_qs(parsed.query)
            try:
                path = _resolve_project_id((query.get("id") or [""])[0], paths=paths)
                result = self.server.project_store().read(path.relative_to(paths.projects).as_posix())
            except Exception as err:
                self._send_json({"error": str(err)}, status=400)
                return
            self._send_json(result)
            return
        if parsed.path == PROJECT_RECEIPT_API:
            query = parse_qs(parsed.query)
            try:
                exporting = (query.get("kind") or [""])[0] == "export"
                store = self.server.export_store() if exporting else self.server.project_store()
                if query.get("request_id"):
                    result = store.outcome(query["request_id"][0])
                    if exporting and result.get("receipt"):
                        target = (paths.exports / result["receipt"]["target_id"]).resolve()
                        result.update(id=_safe_relpath(target), name=target.name)
                else:
                    result = {"current": store.verify((query.get("id") or [""])[0], (query.get("fingerprint") or [""])[0])}
        ...
        if parsed.path == EDITOR_ASSETS_API:
            try:
                ...
                result = {"payload": _read_asset(asset_id, paths=paths)} if asset_id else _asset_entries(paths=paths)
            except (OSError, ValueError, TypeError, OverflowError) as err:
                self._send_json({"error": str(err)}, status=400)
                return
            self._send_json(result)
        ...
        if parsed.path == EDITOR_ASSET_PREVIEW_API:
            try:
                ...
                with self.server.asset_preview_lock:
                    ...
                    if body is None:
                        payload = _read_asset(asset_id, paths=paths)
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == EDITOR_DIAGNOSTICS_API:
            ...
            except DiagnosticBusy:
                self._send_json({"error": "diagnostics_busy"}, status=503)
        ...
        if parsed.path == EDITOR_ASSETS_API:
            ...
            return
        if parsed.path == STARTUP_HELP_API:
            _write_json_atomic(paths.startup_help, {"confirmed": True})
            self._send_json({
                "confirmed": True,
                "marker": str(paths.startup_help),
            })
        ...
        if parsed.path == EDITOR_PREFS_API:
            try:
                ...
                self._send_json({"error": str(err)}, status=400)
                return
            self._send_json({"ok": True, **result, "marker": str(paths.preferences)})
            return
        if parsed.path == EDITOR_THEMES_API:
            try:
                length = int(self.headers.get("Content-Length") or "0")
                ...
                theme_id = _unique_theme_id(_theme_id_from_name(str(data.get("id") or name)), paths=paths)
                target = paths.themes / f"{theme_id}.json"
                payload = {
        ...
        if parsed.path == EDITOR_AUTOSAVE_API:
            ...
            if diagnostics is not None:
                try:
                    cause = data.get("editor_recovery_cause")
                    cause = cause if isinstance(cause, dict) else {}
                    diagnostics.record("checkpoint", **{key: cause[key] for key in
                        ("pageId", "documentId", "commitId", "firstCommitId", "operationId", "commandId", "inputId", "historyId") if key in cause},
                        source="native", revision=result["recovery_revision"],
                        state="committed" if result.get("applied") else "cancelled", duration=(time.perf_counter() - started) * 1000)
                except Exception:
                    pass
            self._send_json({"ok": True, "marker": str(paths.autosave), **result})
            return
        if parsed.path == EDITOR_EXPORT_API:
            ...
            try:
                ...
                store = self.server.export_store()
                with store.lock:
                    target = _unique_editor_export_path(str(data.get("name") or "vinyl"), paths=paths)
                    result = store.save(target.relative_to(paths.exports).as_posix(), payload, request_id=data.get("request_id"))
                self._record_save_commit("saveExport", result, started)
                _record_desktop_change(paths.output_change, target)
            except ProjectSaveError as err:
                self._send_json({"error": str(err), "code": err.code}, status=409)
                return
            except Exception as err:
                self._send_json({"error": str(err)}, status=400)
                return
        ...
        if parsed.path == PROJECT_SAVE_API:
            ...
            try:
                ...
                target = _project_path(project_name, paths=paths)
                target_id = data.get("target_id") if data.get("expected_fingerprint") else None
                result = self.server.project_store().save(target_id or target.relative_to(paths.projects).as_posix(), payload,
                    request_id=data.get("request_id"), expected=data.get("expected_fingerprint"),
                    legacy_overwrite=bool(data.get("overwrite")))
                self._record_save_commit("saveProject", result, started)
                target = (paths.projects / result["receipt"]["target_id"]).resolve()
                _record_desktop_change(paths.project_change, target)
            except ProjectSaveError as err:
                self._send_json({"error": str(err), "code": err.code}, status=409)
                return
            except Exception as err:
            ...
            self._send_json({
                **result,
                "id": target.relative_to(paths.projects).as_posix(),
                "path": str(target),
                "name": target.name,
                "title": project_name,
            })
...
class EditorServer(socketserver.ThreadingTCPServer):
    ...
    def __init__(self, *args, paths: ServerPaths | None = None, **kwargs):
        self.paths = paths if paths is not None else ServerPaths.legacy()
        ...
        self.update_status = {"localVersion": "", "latestVersion": "", "available": False, "checking": False, "checked": False}
    ...
    def project_store(self):
        ...
        with self._projects_lock:
            if self._projects is None:
                self._projects = ProjectStore(paths.projects, _write_json_atomic, EDITOR_PROJECT_MAX_BYTES)
    ...
    def export_store(self):
        ...
        with self._projects_lock:
            if self._exports is None:
                self._exports = ProjectStore(paths.exports, _write_json_atomic, 20 * 1024 * 1024,
                                            receipts=paths.server_marker.parent / "export-receipts")
    ...
    def diagnostics(self):
        ...
        with self._diagnostics_lock:
            if self._diagnostics is None:
                self._diagnostics = EditorDiagnostics(ROOT, paths.server_marker.parent)
    ...
    def recovery_store(self):
        paths = self.paths
        if self._recovery is None or self._recovery.marker != paths.autosave:
    ...
    def store_asset(self, data: object) -> dict:
        ...
        with self.asset_lock:
            if action == "save":
                ...
                target = _asset_path(payload["id"], paths=paths)
            ...
            payload = _read_asset(data.get("id"), paths=paths)
    ...
    def store_preferences(self, data: object) -> dict:
        ...
        if "theme" in data and (not isinstance(data["theme"], str) or not _theme_exists(data["theme"], paths=paths)):
        ...
        if theme is not None and not _theme_exists(theme, paths=paths):
            raise ValueError("invalid editor theme")
        with self.preferences_lock:
            ...
            _write_json_atomic(paths.preferences, payload)
            return payload
        ...
        with self.autosave_lock:
            ...
            if revision and revision <= self.autosave_revision:
                if revision == self.autosave_revision:
                    ...
                    try:
                        stored = json.loads(paths.autosave.read_text(encoding="utf-8"))
                        if stored == expected:
                            store._read(paths.autosave, False)
            ...
            try:
                if clearing and not revision:
                    if self.autosave_revision:
                        return {"applied": False, "recovery_revision": self.autosave_revision}
                    paths.autosave.unlink(missing_ok=True)
...
def _write_server_marker(port: int, session_token: str, *, paths: ServerPaths | None = None) -> None:
    paths = paths if paths is not None else ServerPaths.legacy()
    payload = {
        "service": "kfps-fabric-editor",
        "pid": os.getpid(),
    ...
    _write_json_atomic(paths.server_marker, payload)


def _remove_owned_server_marker(*, paths: ServerPaths | None = None) -> None:
    paths = paths if paths is not None else ServerPaths.legacy()
    try:
        payload = json.loads(paths.server_marker.read_text(encoding="utf-8"))
        if int(payload.get("pid") or -1) == os.getpid():
            paths.server_marker.unlink(missing_ok=True)
    except Exception:
        pass
