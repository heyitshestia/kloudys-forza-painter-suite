...
import time
import uuid
from .manifest import diagnostic_assets, editor_web_root
...
class DiagnosticBusy(Exception):
    """The browser must retain and retry this batch; it was not accepted."""


def clean_runtime_identity(value):
    if not isinstance(value, dict):
        return {}
    result = {}
    for key in ("python", "pyside", "qt", "webengine", "chromium"):
        version = value.get(key)
        if isinstance(version, str) and re.fullmatch(r"[0-9.a-zA-Z+-]{1,48}", version):
            result[key] = version
    if value.get("status") in ("verified", "development"):
        result["status"] = value["status"]
    if value.get("test_debugging") is True:
        result["test_debugging"] = True
    baseline = value.get("baseline")
    if isinstance(baseline, str) and re.fullmatch(r"[a-f0-9]{64}", baseline):
        result["baseline"] = baseline
    for key in ("bits", "files", "bytes", "verification_ms"):
...
KINDS = frozenset((
    "page-start", "action-start", "action-end", "command", "edit", "commit", "checkpoint", "phase", "job", "long-task",
    ...
    "native-reload", "native-close", "native-sample", "renderer-stopped", "close-timeout", "heartbeat-stale",
))
ACTIONS = frozenset((
...
NUMBERS = frozenset((
    ...
    "draws", "totalDraws", "selectedId", "buttons", "modifiers", "clientDrops", "criticalDrops", "editorRevision",
    "operationId", "jobId", "documentGeneration", "queueDepth", "documentId", "commitId", "firstCommitId",
    "commandId", "inputId", "historyId", "workerDuration", "renderDuration", "meshCount", "cachePixels", "cacheCount",
))
...
ENUMS = {
    ...
    "job": frozenset(("recovery", "browserRecovery", "readRecovery", "recoveryHead", "parseFile", "parseText", "fetchJSON", "referenceImage", "saveProject", "saveExport")),
    "renderer": frozenset(("fabric", "gpu-preview", "fallback", "starting")),
    "state": frozenset(("pending", "committed", "finished", "cancelled", "saved", "failed", "idle", "starting", "ready", "closed")),
    ...
    "errorCode": frozenset(("input_too_large", "recovery_too_large", "reference_too_large", "project_too_large", "export_too_large",
        "storage_busy", "storage_closed", "storage_failed", "worker_failed", "worker_timeout", "http_error", "invalid_response",
        "stale_revision", "recovery_head_unavailable", "project_exists", "project_conflict", "save_request_used")),
    "phase": frozenset(("document-build", "document-prewarm", "document-install", "document-retire", "document-present",
        "document-paint", "document-cache", "settled-paint")),
...
def clean_fields(value):
    ...
    for key, item in value.items():
        if key in NUMBERS and type(item) in (int, float) and math.isfinite(item) and abs(item) <= 2**53 - 1:
            ...
            result[key] = item
        elif key == "requestId" and isinstance(item, str) and re.fullmatch(r"[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}", item):
            result[key] = item
        elif key == "pageId" and isinstance(item, str) and re.fullmatch(r"[a-f0-9]{32}", item):
            result[key] = item
    return result
...
class EditorDiagnostics:
    ...
    def accept(self, value):
        ...
        with self._lock:
            ...
            if self._closed.is_set():
                raise DiagnosticBusy()
            serial = self._accepted + 1
            item = {"schema": SCHEMA, "session": self.session, "utc": utc_now(), "serial": serial,
                    "kind": "sample", **packet}
            try:
                self._queue.put_nowait(item)
            except queue.Full:
                raise DiagnosticBusy() from None
            self._accepted = serial
            self._packet = packet
    ...
    def native_tick(self, **fields):
        with self._lock:
            ...
            sample = dict(self._native)
        return self.record("native-sample", **sample)
    ...
    def set_runtime_identity(self, identity):
        with self._lock:
            self._identity["runtime"] = clean_runtime_identity(identity)
    ...
    def _identify(self):
        ...
        try:
            ...
            pass
        try:
            web = editor_web_root(self.root)
        except (OSError, ValueError, KeyError):
            web = None
        for name in ASSETS:
            try:
                identity["assets"][name] = hashlib.sha256((web / name).read_bytes()).hexdigest() if web else "missing"
            except OSError:
                identity["assets"][name] = "missing"
        with self._lock:
            identity["runtime"] = self._identity.get("runtime", {})
    ...
    def _write_batch(self, records):
        ...
        with log.open("a", encoding="utf-8", newline="\n") as handle:
            ...
            handle.flush()
        for record in records:
            if record["kind"] == "native-sample" and record.get("uiLag", 0) < 100:
                continue
...
def support_recent(value):
    """Retain causal outcomes before routine samples within the same 16-record budget."""
    items = [clean_fields(item) for item in value[-48:]] if isinstance(value, list) else []
    important = {"edit", "commit", "checkpoint", "recovery-result", "js-error", "rejection", "console-error",
                 "webgl-lost", "canvas-lost", "renderer-stopped", "native-failed", "close-timeout"}
    def rank(index):
        item = items[index]
        priority = item.get("kind") in important or item.get("state") == "failed" or bool(item.get("requestId"))
        return (priority, index)
    indices = sorted(sorted(range(len(items)), key=rank, reverse=True)[:16])
...
def read_support_diagnostics(root: Path):
    ...
    try:
        ...
        result = {"schema": SCHEMA, "age_seconds": max(0, round(time.time() - updated, 1)),
                  ...
                  "recent": support_recent(snapshot.get("recent", []))}
        identity = snapshot.get("identity") or {}
        result["editor_runtime"] = clean_runtime_identity(identity.get("runtime"))
